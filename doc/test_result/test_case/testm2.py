...
DEFAULT_BASE = os.environ.get("M02_API_URL", "http://localhost:3002/api/v1")
...
def _conversations_list(body) -> list:
    """Normalize list response — never call .get on connection-error strings."""
...
def run_all(base: str) -> List[SmokeResult]:
    ...
    add(
        run_case(
            ...
            predicate=lambda b, _: (
                ...
                or (
                    isinstance(b, dict)
                    and isinstance(b.get("conversations"), list)
                    and len(b.get("conversations") or []) == 0
                )
            ),
        )
    )
    ...
    add(
        run_case(
            ...
            tenant=TENANT_A,
            predicate=lambda b, _: len(_conversations_list(b)) <= 5,
        )
    )
    add(
        run_case(
    ...
    add(
        run_case(
            ...
            tenant=TENANT_A,
            predicate=lambda b, _: all(
                c.get("channel") == "email" for c in _conversations_list(b)
            ),
        )
    )
    add(
        run_case(
            ...
            tenant=TENANT_A,
            predicate=lambda b, _: isinstance(_conversations_list(b), list),
        )
    )
    add(
        run_case(
            ...
            tenant=TENANT_A,
            predicate=lambda b, _: len(_conversations_list(b)) == 0,
        )
    )
...
def _probe_api(base: str) -> bool:
    from _http import request as http_request

    root = base.rstrip("/").removesuffix("/api/v1") if "/api/v1" in base else base.rstrip("/")
    health_urls = [
        f"{root}/api/v1/conversation-intelligence/conversations",
        f"{base.rstrip('/')}/conversation-intelligence/conversations",
    ]
    for url in health_urls:
        status, _, _ = http_request("GET", url, tenant=TENANT_A, timeout=3)
        if status > 0:
            return True
    return False
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="API base (default http://localhost:3002/api/v1)",
    )
    args = parser.parse_args()

    print("M02 Deep Smoke — Conversation Intelligence")
    print(f"Base: {args.base_url}")
    if not _probe_api(args.base_url):
        print(
            "\nERROR: API not reachable. Start Docker Postgres, then:\n"
            "  $env:PORT='3002'; $env:DISABLE_REDIS='true'; "
