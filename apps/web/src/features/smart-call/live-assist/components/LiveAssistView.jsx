...
import {
  generateLiveAssistInsights,
  transcribeAudioBlob,
  analyzeCompetitorThreat,
  generateStrategicSummary,
  createLiveCallDbSession,
  storeLiveCallChunk,
  completeLiveCallDbSession,
  hasServerLlmKeys,
} from "../live-assist-api";
...
import {
  ...
  shouldTriggerProactiveCompetitor,
  resetAlertCooldowns,
  extractNamesFromTranscript,
} from "../services/localEventEngine";
...
export default function LiveAssistView({
  ...
  const apiKey = hasServerLlmKeys() ? 'server' : '';
  const openRouterKey = '';
  const deals = dealContext ? [dealContext] : [];
  const isWideMode = true;
  const initialDealId = dealContext?.id || 'new_call';
  ...
  useEffect(() => {
    if (dealContext) {
      selectedDealRef.current = dealContext;
    }
  }, [dealContext]);

  useEffect(() => {
    if (!autoStartCapture || !apiKey) return;
  ...
  }, [autoStartCapture, apiKey]);

  // Postgres session (replaces Supabase)
  async function createSession() {
    if (!backendSessionId) return null;
    try {
      const id = await createLiveCallDbSession({
        sessionId: backendSessionId,
        contactId: dealContext?.contactId,
        dealCompany: selectedDealRef.current?.company || dealContext?.company,
        clientName: dealContext?.contactName || selectedDealRef.current?.contact,
        sessionName: `Live Call - ${new Date().toLocaleString()}`,
      });
      return id || backendSessionId;
    } catch (e) {
      console.warn("[LiveAssist] Session create error:", e);
  ...
  async function storeChunkSummary(sid, chunk) {
    if (!sid) return;
    try {
      await storeLiveCallChunk(sid, chunk);
    } catch (e) {
      console.warn("[LiveAssist] Summary store error:", e);
    }
  }
  ...
  async function stopCapture() {
    ...
    try {
      ...
      if (summary) {
        ...
        if (sid) {
          try {
            await completeLiveCallDbSession(sid, {
              finalSummary: summary,
              totalSegments: segmentsRef.current.length,
              salesRepName: insights.speakerIdentification?.salesRepName || "Sales Rep",
              clientName: insights.speakerIdentification?.clientName || "Client",
              transcript: segmentsRef.current,
            });
            if (onEndSession) {
              await onEndSession(summary, segmentsRef.current);
            }
  ...
  function localEngineTick() {
    ...
    if (delta.hasNewCompetitor && currentSignals.competitor.matched.length > 0) {
      ...
      if (newCompetitors.length > 0) {
        ...
        analyzeCompetitorThreat(competitorName, rolling).then(result => {
          if (result) {
  ...
  async function runAnalysis(triggerReason = 'scheduled', triggerSignals = [], requestType = 'full') {
    ...
    try {
      ...
      const data = await generateLiveAssistInsights({
        ...
        dealContext: selectedDealRef.current || dealContext,
  ...
  async function drainQueue() {
    ...
    try {
      while (chunkQueueRef.current.length > 0) {
        ...
        try {
          const result = await transcribeAudioBlob(transcribeBlob);
  ...
  async function startCapture() {
    ...
    if (!apiKey) {
      setError("Live Assist is unavailable: set GROQ_API_KEY (and optional OPENROUTER_API_KEY) on the API server.");
      return;
    }
