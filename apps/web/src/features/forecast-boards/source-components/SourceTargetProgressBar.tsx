...
export default function SourceTargetProgressBar({ quota, closed, commit = 0 }: SourceTargetProgressBarProps) {
  if (!quota) return null;

  const attainmentPct = calculateTargetAttainment(closed, commit, quota) ?? 0;
  ...
  return (
    <div className="flex flex-col gap-1 min-w-[116px]">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[10px] text-gray-500">{formatCurrency(quota, true)}</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-1">
        <div
          className="h-1 rounded-full transition-all duration-300 bg-blue-600"
          style={{ width: `${pct}%` }}
        />
      </div>
      {isOverTarget && (
