...
exports.buildFeedbackFromTranscript = buildFeedbackFromTranscript;
const m01_transcript_review_util_1 = require("./m01-transcript-review.util");
...
function buildFeedbackFromTranscript(record) {
    const review = (0, m01_transcript_review_util_1.buildReviewFromTranscript)(record);
    ...
    if (review.overallScore < 82)
        tags.push('Needs Coaching');
    if (highlights.some((h) => /objection/i.test(String(h.label ?? ''))))
        tags.push('Objection Handling');
    if (highlights.some((h) => /pricing|budget|cost/i.test(String(h.label ?? ''))))
        tags.push('Pricing');
    if (highlights.some((h) => /pain|discovery|product/i.test(String(h.label ?? ''))))
        tags.push('Discovery');
    if (tags.length === 0)
        tags.push('Discovery');
    const summary = transcript.summary?.trim() || 'Call transcript analyzed for coaching feedback.';
    const coachingNotes = `${summary} Scorecard overall: ${review.overallScore}/100. ${improvementAreas.length > 0
        ? 'Priority focus: ' + improvementAreas.slice(0, 2).join(' ')
        : 'Maintain current discovery and closing habits on upcoming calls.'}`;
    const recommendedActions = uniqueNonEmpty([...nextSteps, ...improvementAreas.map((s) => s.replace(/^Ask about /i, 'Practice '))], 5);
...
//# sourceMappingURL=m01-transcript-feedback.util.js.map