...
exports.buildReviewFromTranscript = buildReviewFromTranscript;
...
function repUtterances(utterances) {
    return utterances.filter((u) => /rep/i.test(String(u.speaker ?? '')));
}
function customerUtterances(utterances) {
...
function findFirstMatch(utterances, patterns) {
    for (const u of utterances) {
        if (matchesAny(u.text ?? '', patterns))
            return u;
    }
    return null;
}
function countMatches(utterances, patterns) {
...
function sectionScoreFromQuestions(questions) {
    if (questions.length === 0)
        return 0;
    const avg = questions.reduce((sum, q) => sum + q.score, 0) / questions.length;
    return Math.round(avg * 20);
}
function buildQuestion(questionText, score, evidence, comment, suggestion) {
...
function highlightEvidence(highlights, labelPattern) {
...
function buildOpeningSection(repLines, allLines) {
    ...
    const questions = [
        buildQuestion('Did the rep introduce themselves and company clearly?', introScore, intro, intro
            ? `Rep introduction found: "${truncate(intro.text ?? '', 120)}"`
            : 'No clear introduction detected in the opening exchanges.', 'Open with your name, company, and reason for the call within the first 30 seconds.'),
        buildQuestion('Did the rep build rapport with the prospect?', rapportScore, rapport, rapport
            ? `Rapport-building language at ${formatTs(rapport.startMs ?? 0)}: "${truncate(rapport.text ?? '', 100)}"`
            : 'Limited rapport-building phrases in the opening.', 'Use a brief personal greeting and acknowledge the prospect’s situation before pitching.'),
        buildQuestion('Was the purpose of the call established early?', purposeScore, purpose, purpose
            ? `Purpose stated: "${truncate(purpose.text ?? '', 120)}"`
            : 'Call purpose was not clearly established in early rep turns.', 'State the call objective explicitly — e.g. discovery, evaluation, or next-step planning.'),
    ];
    return { sectionName: 'Opening', sectionScore: sectionScoreFromQuestions(questions), questions };
}
function buildDiscoverySection(repLines, customerLines, fullText, highlights) {
    ...
    let painScore = 1;
    if (painCustomer && painRep)
        painScore = 5;
    else if (painCustomer || painRep)
        painScore = 4;
    else if (matchesAny(fullText, [/need better integration/i, /looking for something/i]))
        painScore = 3;
    ...
    const pricingEvidence = findFirstMatch(repLines, [
        /\$[\d,]+/i,
        /cost/i,
        /pricing/i,
        /estimate/i,
    ...
    const questions = [
        buildQuestion('Did the rep uncover pain points?', painScore, painEvidence, painEvidence
            ? `Pain or need discussed: "${truncate(painEvidence.text ?? '', 120)}"`
            : 'No explicit pain points surfaced in the conversation.', 'Ask open questions about current workflow challenges before presenting features.'),
        buildQuestion('Were open-ended discovery questions used?', openScore, openEvidence, openEvidence
            ? `Discovery question at ${formatTs(openEvidence.startMs ?? 0)}: "${truncate(openEvidence.text ?? '', 100)}"`
            : `Rep asked ${openQuestions} discovery-style question(s); aim for 3+ per call.`, 'Use SPIN-style open questions — "What challenges…", "How does your team…"'),
        buildQuestion('Was pricing or budget discussed appropriately?', pricingScore, pricingEvidence, pricingEvidence
            ? `Pricing discussed: "${truncate(String(pricingEvidence.text ?? ''), 120)}"`
            : 'Pricing was not addressed when the prospect raised cost concerns.', 'When budget comes up, anchor value before price and confirm decision authority.'),
        buildQuestion('Did the rep confirm next steps or decision process?', decisionScore, decisionEvidence, decisionEvidence
            ? `Next-step language: "${truncate(decisionEvidence.text ?? '', 120)}"`
            : 'Decision process and concrete next steps were not fully confirmed.', 'Close discovery by asking who else is involved and agreeing on a specific next action.'),
    ];
    return { sectionName: 'Discovery', sectionScore: sectionScoreFromQuestions(questions), questions };
}
function buildObjectionSection(repLines, customerLines, highlights) {
    ...
    const objectionResponse = objectionCustomer
        ? repLines.find((u) => (u.startMs ?? 0) > (objectionCustomer.startMs ?? 0) &&
            matchesAny(u.text ?? '', [
                /support/i,
                /discount/i,
                /flexibility/i,
    ...
    if (objectionCustomer && objectionResponse)
        objectionScore = 5;
    else if (objectionResponse || highlightEvidence(highlights, /objection/i))
        objectionScore = 4;
    ...
    const questions = [
        buildQuestion('Did the rep handle objections effectively?', objectionScore, objectionResponse ?? objectionCustomer, objectionCustomer
            ? objectionResponse
            ...
            : 'No major objections detected in this call.', 'Acknowledge the concern, then respond with specific proof points or flexibility options.'),
        buildQuestion('Were product differentiators clearly explained?', diffScore, diffEvidence, diffEvidence
            ? `Differentiators cited: "${truncate(diffEvidence.text ?? '', 120)}"`
            : 'Product differentiation was generic; tie features to prospect-stated needs.', 'Link each differentiator to a pain point the customer already mentioned.'),
    ];
    return {
...
function buildClosingSection(repLines, transcript, repPct) {
    ...
    let nextScore = 2;
    if (nextStepEvidence && nextSteps.length > 0)
        nextScore = 5;
    else if (nextStepEvidence || nextSteps.length > 0)
        nextScore = 4;
    else if (matchesAny(fullTranscriptText(transcript), [/demo|proposal|send/i]))
        nextScore = 3;
    ...
    const questions = [
        buildQuestion('Were clear next steps agreed with the prospect?', nextScore, nextStepEvidence, nextStepEvidence
            ? `Next step confirmed: "${truncate(nextStepEvidence.text ?? '', 120)}"`
            : nextSteps.length
                ? `Transcript next steps: ${nextSteps.slice(0, 2).join('; ')}`
                : 'No explicit next-step commitment at close.', 'End with a specific action, owner, and timeframe — e.g. demo invite by EOD.'),
        buildQuestion('Did the rep confirm follow-up deliverables?', followScore, followUpEvidence, followUpEvidence
            ? `Follow-up promised: "${truncate(followUpEvidence.text ?? '', 120)}"`
            : 'Follow-up deliverables were not clearly committed.', 'Name the exact asset you will send (proposal, recap, security docs) and when.'),
        buildQuestion('Was talk/listen balance appropriate for discovery?', listenScore, null, repPct != null
            ? `Rep talk ratio: ${Math.round(repPct)}% / Customer: ${Math.round(100 - repPct)}%`
            : 'Talk ratio unavailable for this transcript.', listenSuggestion || 'Monitor talk ratio — discovery calls should leave room for the buyer to share context.'),
    ];
...
function formatReviewDate(record) {
...
function buildReviewFromTranscript(record) {
    const transcript = record.transcript;
    const utterances = transcript?.utterances ?? [];
    if (!utterances.length && !transcript?.fullText)
        return null;
    ...
    const overallScore = Math.round(sections.reduce((sum, s) => sum + s.sectionScore, 0) / sections.length);
    return {
...
//# sourceMappingURL=m01-transcript-review.util.js.map