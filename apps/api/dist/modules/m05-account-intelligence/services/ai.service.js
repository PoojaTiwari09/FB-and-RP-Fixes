...
let AiService = class AiService {
    ...
    constructor(configService) {
        ...
        this.db = (0, supabase_1.getSupabase)();
    }
    async buildContext(companyHubspotId, scope, periodDays) {
    ...
    extractCitations(reply, activities) {
        const citations = [];
        const seenDates = new Set();
        for (const act of activities.slice(0, 30)) {
            const ts = (act.timestamp || '').substring(0, 10);
            if (!ts || seenDates.has(ts))
                continue;
    ...
    async callGroq(messages, responseFormat) {
        if (!this.groqApiKey) {
    ...
    async getCachedBrief(companyHubspotId, scope, periodDays) {
        try {
    ...
    async saveBriefCache(companyHubspotId, scope, periodDays, brief) {
        try {
            await this.db
                .from('ai_briefs_cache')
                .upsert({
