...
exports.parseDueDateTime = parseDueDateTime;
exports.resolveDueDateTime = resolveDueDateTime;
exports.buildTaskTitle = buildTaskTitle;
exports.resolveSequenceName = resolveSequenceName;
exports.resolveSequenceStep = resolveSequenceStep;
...
function resolveDueDateTime(task) {
...
function buildTaskTitle(task) {
    ...
    if (channel === 'CALL')
        return `Call with ${task.contactName}`;
    if (channel === 'EMAIL')
        return `Email follow-up — ${task.contactName}`;
    if (channel === 'LINKEDIN')
        return `LinkedIn outreach — ${task.contactName}`;
    return `${task.contactName} — ${task.companyName}`;
}
function resolveSequenceName(task) {
    return task.sequenceName?.trim() || task.workflowName?.trim() || '';
}
function resolveSequenceStep(task) {
    if (task.sequenceStep?.trim())
        return task.sequenceStep.trim();
    if (task.workflowStep?.trim())
        return `Step ${task.workflowStep.trim()}`;
    return '';
}
//# sourceMappingURL=m08-task-format.util.js.map