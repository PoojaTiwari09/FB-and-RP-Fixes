...
async function runTests() {
    ...
    const service = app.get(m08_service_1.M08SalesEngagementService);
    ...
    try {
        ...
        try {
            ...
            assert(play.steps.length === 2, 'Two steps registered in playbook');
            ...
            assert(cloned.steps.length === 2, 'Cloned steps successfully preserved');
        ...
        try {
            const fakeControllerCheck = (role) => {
            ...
            try {
                ...
                assert(e instanceof common_1.ForbiddenException, 'Enforced RBAC: Representatives rejected with ForbiddenException on Admin endpoints');
                passed++;
            }
        }
        catch (e) {
