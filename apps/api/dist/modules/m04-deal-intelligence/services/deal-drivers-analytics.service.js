...
exports.DealDriversAnalyticsService = void 0;
const common_1 = require("@nestjs/common");
const deal_catalog_service_1 = require("./deal-catalog.service");
const deal_driver_types_1 = require("../interfaces/deal-driver.types");
...
let DealDriversAnalyticsService = class DealDriversAnalyticsService {
    catalog;
    constructor(catalog) {
    ...
    detectWarnings(deal) {
        ...
        if (!nextStep)
            warnings.push('no_next_step');
        if (Number(deal.contacts ?? 0) < 2)
            warnings.push('single_threaded');
        if (!deal.closeDate)
            warnings.push('no_close_plan');
        if (this.daysSince(String(deal.lastActivity ?? '')) > 14)
            warnings.push('stale_gt14d');
        const cats = deal._meddpiccCategories || [];
        if (!cats.includes('champion') || Number(deal.meddpiccScore ?? 0) < 45) {
            warnings.push('champion_left');
        }
        return warnings;
    }
    riskScore(deal, warnings) {
        let score = 30 + warnings.length * 15;
        score += Math.min(this.daysSince(String(deal.lastActivity ?? '')), 30);
        if (Number(deal.amount ?? 0) > 200000)
            score += 10;
    ...
    async filterDeals(deals, params) {
    ...
    async getSummary(params) {
        ...
        for (const d of active) {
            for (const w of this.detectWarnings(d))
                counts[w]++;
        }
        ...
        return {
            ...
            highestRiskWarning: {
                label: top ? deal_driver_types_1.WARNING_LABELS[top[0]] : 'No Next Step',
    ...
    async getComparePeriods(params) {
        const matrix = await this.getRiskMatrix(params);
        const warningKeys = ['no_next_step', 'single_threaded', 'no_close_plan', 'stale_gt14d', 'champion_left'];
        const fieldMap = {
        ...
        const comparisons = warningKeys.map((wt) => {
            const vals = matrix.reps.map((r) => r.warnings[fieldMap[wt]]?.pct ?? 0);
    ...
    async getAiInsights(params) {
        ...
        const insights = atRisk.map((x, i) => ({
            ...
            recommendation: `Review ${deal_driver_types_1.WARNING_LABELS[x.w[0]]} in next 1-on-1`,
            rep: this.repKey(x.d),
            account: String(x.d.dealName || x.d.name),
            score: x.score,
            tags: x.w.map((w) => deal_driver_types_1.WARNING_LABELS[w]),
        }));
    ...
    async getDrilldown(params) {
        ...
        return {
            ...
            repSidebar: {
                ...
                avgCloseRate: 31.2,
                aiCoachingTip: 'Map all stakeholders before next call. Identify champion and economic buyer for each flagged deal.',
            },
        };
    }
    closeDateStatus(closeDate) {
        if (!closeDate)
            return 'ok';
...
exports.DealDriversAnalyticsService = DealDriversAnalyticsService;
exports.DealDriversAnalyticsService = DealDriversAnalyticsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [deal_catalog_service_1.DealCatalogService])
], DealDriversAnalyticsService);
//# sourceMappingURL=deal-drivers-analytics.service.js.map