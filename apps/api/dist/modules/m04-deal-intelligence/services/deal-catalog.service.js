...
var DealCatalogService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.DealCatalogService = void 0;
const common_1 = require("@nestjs/common");
const hubspot_service_1 = require("./hubspot.service");
const deals_service_1 = require("./deals.service");
const deal_meddpicc_service_1 = require("./deal-meddpicc.service");
...
let DealCatalogService = DealCatalogService_1 = class DealCatalogService {
    hubSpotService;
    dealsService;
    meddpiccService;
    logger = new common_1.Logger(DealCatalogService_1.name);
    constructor(hubSpotService, dealsService, meddpiccService) {
        this.hubSpotService = hubSpotService;
        this.dealsService = dealsService;
        this.meddpiccService = meddpiccService;
    }
    calculateMeddpiccFromTexts(texts) {
    ...
    analyzeMeddpiccFromTexts(texts) {
        const text = texts.join(' ').toLowerCase();
        const categories = {
            metrics: [
                'metrics', 'quantifiable', 'business metrics', 'kpi', 'roi', 'return on investment',
    ...
    deriveContactsFromTranscripts(texts) {
        const text = texts.join(' ').toLowerCase();
        const rolePatterns = [
    ...
    generateNextStep(matchedCategories) {
    ...
    extractAnswerForCategory(texts, category) {
    ...
    async enrichDealWithTranscriptMeddpicc(deal) {
    ...
    async loadDeals(limit = 100) {
        try {
            ...
            return { deals: enrichedDeals, boards, isMock: false };
        }
        catch (error) {
    ...
    async findDealById(dealId) {
        try {
            ...
            if (!mock)
                throw new common_1.NotFoundException(`Deal ${dealId} not found in Deal Boards`);
            const enriched = await this.enrichDealWithTranscriptMeddpicc(mock);
            return enriched;
        }
    }
    async resolveBoardForDeal(dealId) {
        ...
        if (deal) {
            ...
            if (board)
                return { boardId: board.boardId, boardName: board.name };
        }
    ...
    async assertDealExists(dealId) {
        return this.findDealById(dealId);
    }
};
exports.DealCatalogService = DealCatalogService;
exports.DealCatalogService = DealCatalogService = DealCatalogService_1 = __decorate([
...
//# sourceMappingURL=deal-catalog.service.js.map