...
exports.defaultVoices = defaultVoices;
exports.mapTrainingListItem = mapTrainingListItem;
exports.mapTrainingSetup = mapTrainingSetup;
exports.mapMessages = mapMessages;
exports.mapSessionState = mapSessionState;
...
function mapTrainingListItem(scenario, assignment) {
...
function mapTrainingSetup(scenario) {
    ...
    if (ctxLower.includes('cloud') || ctxLower.includes('migration'))
        topic = 'cloud migration';
    else if (ctxLower.includes('sales') || ctxLower.includes('crm'))
        topic = 'sales platform';
    else if (ctxLower.includes('security') || ctxLower.includes('compliance'))
        topic = 'security solution';
    else if (ctxLower.includes('automat'))
        topic = 'automation platform';
...
function mapSessionState(session, scenario, messages) {
    return {
...
function mapResults(session, feedback) {
    if (!feedback) {
        return {
    ...
    return {
        ...
        performanceTags: (feedback.strengths || []).slice(0, 3).map((s, i) => ({
            id: `tag_${i}`,
            label: s,
            type: 'positive',
        })),
        ...
        transcript: mapMessages(typeof session.messages_json === 'string'
            ? JSON.parse(session.messages_json)
            : session.messages_json),
        resultsReady: true,
    };
}
//# sourceMappingURL=m09-frontend-trainings.mapper.js.map