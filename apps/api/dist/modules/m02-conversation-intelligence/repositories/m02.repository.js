...
var M02ConversationIntelligenceRepository_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.M02ConversationIntelligenceRepository = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../database/prisma.service");
const DEV_SEED_TENANT_ID = '00000000-0000-0000-0000-000000000001';
let M02ConversationIntelligenceRepository = class M02ConversationIntelligenceRepository {
    static { M02ConversationIntelligenceRepository_1 = this; }
    prisma;
    static logger = new common_1.Logger(M02ConversationIntelligenceRepository_1.name);
    static demoCorpusByTenant = new Map();
    static savedSearchesByTenant = new Map();
    static syncLogsByTenant = new Map();
    constructor(prisma) {
        this.prisma = prisma;
    }
    get callRecordDelegate() {
        const p = this.prisma;
        return p?.callRecord ?? p?.m01Call ?? null;
    }
    get transcriptDelegate() {
        const p = this.prisma;
    ...
    demoCorpusFor(tenantId) {
        const cached = M02ConversationIntelligenceRepository_1.demoCorpusByTenant.get(tenantId);
        if (cached)
            return cached;
        const corpus = this.generateSampleCorpus(tenantId);
        M02ConversationIntelligenceRepository_1.demoCorpusByTenant.set(tenantId, corpus);
        M02ConversationIntelligenceRepository_1.savedSearchesByTenant.set(tenantId, this.generateSampleSavedSearches(tenantId));
        M02ConversationIntelligenceRepository_1.syncLogsByTenant.set(tenantId, this.generateSampleSyncLogs(tenantId));
        M02ConversationIntelligenceRepository_1.logger.log(`Generated ${corpus.length} demo conversations for tenant ${tenantId} (DB delegates unavailable).`);
        return corpus;
    }
    mapCallToConversation(c) {
    ...
    async findAllConversations(tenantId) {
        if (!tenantId)
        ...
        return [
            ...calls.map((c) => this.mapCallToConversation(c)),
            ...emails.map((e) => this.mapEmailToConversation(e)),
        ];
    }
    async findConversationById(id, tenantId) {
        ...
        try {
            ...
            if (callDelegate?.findFirst) {
                ...
                if (c)
                    return this.mapCallToConversation(c);
            }
            ...
            M02ConversationIntelligenceRepository_1.logger.warn(`DB fetch for single conversation failed: ${err.message}`);
        }
        if (tenantId !== DEV_SEED_TENANT_ID)
            return undefined;
        return this.demoCorpusFor(tenantId).find((c) => c.id === id);
    }
    async findSavedSearches(tenantId, _userId) {
        if (!tenantId)
            throw new Error('tenantId is required');
        try {
            ...
            if (delegate?.findMany) {
                ...
                if (searches.length > 0) {
                    return searches.map((s) => ({
                        ...
                        filters: typeof s.filters === 'string'
            ...
            M02ConversationIntelligenceRepository_1.logger.warn(`DB fetch for saved searches failed: ${err.message}`);
        }
        if (tenantId !== DEV_SEED_TENANT_ID)
            return [];
        return M02ConversationIntelligenceRepository_1.savedSearchesByTenant.get(tenantId) ?? [];
    }
    async createSavedSearch(data, tenantId, userId) {
        if (!tenantId)
            throw new Error('tenantId is required');
        if (!userId)
        ...
        try {
            ...
            M02ConversationIntelligenceRepository_1.logger.warn(`DB create saved search failed: ${err.message}`);
        }
        const newSearch = {
        ...
        const list = M02ConversationIntelligenceRepository_1.savedSearchesByTenant.get(tenantId) ?? [];
        list.push(newSearch);
        M02ConversationIntelligenceRepository_1.savedSearchesByTenant.set(tenantId, list);
        return newSearch;
    }
    async findSyncLogs(tenantId) {
        if (!tenantId)
            throw new Error('tenantId is required');
        try {
            ...
            M02ConversationIntelligenceRepository_1.logger.warn(`DB fetch sync logs failed: ${err.message}`);
        }
        if (tenantId !== DEV_SEED_TENANT_ID)
            return [];
        return M02ConversationIntelligenceRepository_1.syncLogsByTenant.get(tenantId) ?? [];
    }
    async createSyncLog(data, tenantId) {
        if (!tenantId)
            throw new Error('tenantId is required');
        if (!data.entityId) {
        ...
        try {
            ...
            M02ConversationIntelligenceRepository_1.logger.warn(`DB create sync log failed: ${err.message}`);
        }
        const newLog = {
        ...
        const list = M02ConversationIntelligenceRepository_1.syncLogsByTenant.get(tenantId) ?? [];
        list.push(newLog);
        M02ConversationIntelligenceRepository_1.syncLogsByTenant.set(tenantId, list);
        return newLog;
    }
    async safeFindCalls(tenantId) {
        ...
        try {
            ...
            M02ConversationIntelligenceRepository_1.logger.warn(`findAllConversations.calls failed: ${err.message}`);
            return [];
        }
    }
    async safeFindEmails(tenantId) {
    ...
    static DEV_SEED_USER_ID = '00000000-0000-0000-0000-000000000002';
...
exports.M02ConversationIntelligenceRepository = M02ConversationIntelligenceRepository;
exports.M02ConversationIntelligenceRepository = M02ConversationIntelligenceRepository = M02ConversationIntelligenceRepository_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], M02ConversationIntelligenceRepository);
//# sourceMappingURL=m02.repository.js.map