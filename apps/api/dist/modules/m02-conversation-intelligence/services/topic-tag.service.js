"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TopicTagService = void 0;
const common_1 = require("@nestjs/common");
const topic_repository_1 = require("../repositories/topic.repository");
let TopicTagService = class TopicTagService {
    topicRepository;
    constructor(topicRepository) {
        this.topicRepository = topicRepository;
    }
    async getTagsForConversation(conversationId) {
        return this.topicRepository.getTagsForConversation(conversationId);
    }
    async addManualTag(tenantId, conversationId, topicName, explanation = 'Manually added by user') {
        return this.topicRepository.createTopicTag({
            callId: conversationId,
            tenantId,
            topicName,
            source: 'manual',
            confidenceScore: 1.0,
            explanation,
        });
    }
    async deleteTag(tagId) {
        return this.topicRepository.deleteTag(tagId);
    }
    async deleteTagsForConversation(conversationId) {
        return this.topicRepository.deleteTagsForConversation(conversationId);
    }
};
exports.TopicTagService = TopicTagService;
exports.TopicTagService = TopicTagService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [topic_repository_1.TopicRepository])
], TopicTagService);
//# sourceMappingURL=topic-tag.service.js.map