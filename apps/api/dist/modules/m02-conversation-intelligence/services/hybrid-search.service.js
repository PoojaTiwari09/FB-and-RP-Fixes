...
let HybridSearchService = HybridSearchService_1 = class HybridSearchService {
    ...
    async executePostgresFtsSearch(tenantId, query) {
        if (!query)
            return [];
        try {
            const rows = await this.prisma.$queryRawUnsafe(`SELECT t.id              AS "transcriptId",
                t."callId"        AS "callId",
                cr.title          AS "title",
    ...
    async executePgvectorSearch(tenantId, query) {
        if (!query)
            return [];
        try {
            const rows = await this.prisma.$queryRawUnsafe(`SELECT id           AS "entityId",
