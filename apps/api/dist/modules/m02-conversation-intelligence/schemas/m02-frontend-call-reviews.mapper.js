...
exports.scorecardSectionsTemplate = scorecardSectionsTemplate;
exports.mapReviewListItem = mapReviewListItem;
exports.mapReviewDetail = mapReviewDetail;
function formatMmSs(totalSeconds) {
    const m = Math.floor(totalSeconds / 60);
    const s = totalSeconds % 60;
    return `${m}:${String(s).padStart(2, '0')}`;
}
function scorecardSectionsTemplate() {
    return [
        {
            sectionId: 'sec_01',
        ...
        },
        {
            ...
            questions: [
                {
                    questionId: 'fit_1',
                    questionText: 'Did the rep connect features to customer pain points?',
                    required: true,
                    answerType: 'yes_no',
                    aiSuggestion: true,
                    aiConfidence: 85,
                    aiSuggestedAnswer: true,
                    transcriptRef: '18:20',
                    transcriptSnippet: 'Based on what you mentioned about manual data entry, our workflow automation can eliminate that completely...',
                },
                {
                    questionId: 'fit_2',
                    questionText: 'Rate the quality of the solution presentation',
                    required: true,
                    answerType: 'rating',
                    aiSuggestion: true,
                    aiConfidence: 80,
                    aiSuggestedAnswer: 4,
                    transcriptRef: '20:15',
                    transcriptSnippet: 'The solution demo was well presented and matched prospects\' expectations.',
                },
            ],
        },
        {
            ...
            questions: [
                {
                    questionId: 'obj_1',
                    questionText: 'Did the rep address objections effectively?',
                    required: true,
                    answerType: 'yes_no',
                    aiSuggestion: true,
                    aiConfidence: 70,
                    aiSuggestedAnswer: true,
                    transcriptRef: '22:10',
                    transcriptSnippet: 'Sarah addressed CRM integration concerns nicely.',
                },
                {
                    questionId: 'obj_2',
                    questionText: 'How well did the rep summarize next steps?',
                    required: true,
                    answerType: 'rating',
                    aiSuggestion: true,
                    aiConfidence: 75,
                    aiSuggestedAnswer: 4,
                    transcriptRef: '28:45',
                    transcriptSnippet: 'Briefly discussed next steps but did not lock in a calendar invite on the call.',
                },
            ],
        },
    ];
}
function mapReviewListItem(review) {
    return {
...
function mapReviewDetail(review, call) {
    ...
    return {
        ...
        participants: (call?.participants ?? []).map((name) => ({
            name: name.replace(/\s*\(.*\)/, ''),
...
//# sourceMappingURL=m02-frontend-call-reviews.mapper.js.map