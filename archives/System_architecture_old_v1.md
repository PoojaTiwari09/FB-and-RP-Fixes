...
<!-- ADDED: Visual lifecycle diagram — makes the linear dependency crystal clear at a glance -->
...
<!-- ADDED: "Critical Dependencies" column — makes it immediately obvious which modules 
     must be deployed before a downstream stage can function. -->

> **Developer Rule: Upstream Dependency Check**
>
> Before starting work on any feature in Stage N:
>
> 1. Confirm all modules in **Critical Dependencies** column are deployed and healthy
...
<!-- ⚠️ CORRECTED: The original text created a contradiction. Modules are NOT currently 
     "unit[s] of deployment" (Modular Monolith deploys together). They ARE units of 
     logical separation, testing isolation, and commercial packaging. Fixed below. -->
...
<!-- ADDED: Phase status columns — makes deployment reality vs. commercial promise crystal clear -->

```mermaid
flowchart TD
  M01["M-01 Data Ingestion ✅"] --> M03["M-03 Revenue Graph 🔴 Priority #1"]
  ...
  M06 --> M08["M-08 Execution & Automation"]
  M07 --> M09["M-09 Forecasting"]
  M08 --> M09
  M09 --> M10["M-10 Performance & Coaching"]
  M02["M-02 Sales Engagement 🟡"] -.-> M03

  classDef deployed fill:#c8e6c9
  classDef extraction1 fill:#fff3e0
  classDef extraction2 fill:#ffebee
  classDef extraction3 fill:#e3f2fd
  classDef planned fill:#f5f5f5
...
<!-- ADDED: Phase-aware dependency visualization — shows what's deployable NOW vs. LATER -->

**Phase 1-2 Deployable Path (M-01 → M-03 only)**

```mermaid
flowchart TD
  M01["M-01 Data Ingestion ✅ Deployed"] --> M03["M-03 Revenue Graph 🔴 Phase 3 Priority #1"]
  M02["M-02 Sales Engagement 🟡 Planned"] -.->|Phase 2| M03
  
  classDef deployed fill:#c8e6c9
  classDef phase3priority1 fill:#fff3e0
  classDef planned fill:#f5f5f5
  
  class M01 deployed
  class M03 phase3priority1
  class M02 planned
```
...
```mermaid
flowchart TD
  M01["M-01 Data Ingestion"] --> M03["M-03 Revenue Graph"]
  M03 --> M04["M-04 Conversation Intelligence"]
  M04 --> M05["M-05 Smart Tracking"]
  M05 --> M06["M-06 Insight Generation"]
  
  M06 --> M07["M-07 Deal Management"]
  M06 --> M08["M-08 Execution & Automation"]
...
flowchart LR
  ...
  subgraph CONF["Conferencing & Call Recording 🟢 Webhook Inbound Only"]
    ZOOM["Zoom"]
    GMEET["Google Meet"]
  ...
  subgraph EMAIL["Email & Calendar 🔄 Bidirectional"]
    GMAIL["Gmail"]
  ...
  classDef inbound fill:#e8f5e8
  classDef outbound fill:#fff3e0
  classDef bidir fill:#e3f2fd
...
flowchart TB
  ...
  subgraph EXT["🔌 External Integrations"]
    CRM["CRM (SF/HubSpot/Dynamics)"]
    CONF["Conferencing (Zoom/Meet/Teams)"]
    EMAIL["Email (Gmail/Outlook)"]
    WARE["Data Warehouses\n(Snowflake/BQ/S3/Redshift)"]
    AIEXT["AI APIs\n(OpenAI/AssemblyAI)"]
  end
  ...
  API <-->|"Internal HTTP\n(sync + circuit breaker)"| AIS
  API <-->|"Internal HTTP\n(sync + circuit breaker)"| TS
...
| **Transcription** | FastAPI + Whisper       | ASR, diarization, timestamping            | Audio processing pipeline           | CRM sync, business rules  |
| **Event Bus**     | BullMQ + Redis          | Async inter-module communication          | Job queues, dead-letter             | Business logic            |
| **PostgreSQL**    | Supabase + RLS          | Relational data + tenant isolation        | Transactional data, audit logs      | Analytics, search indexes |
...
| **pgvector**      | Supabase extension      | Embedding store for RAG + semantic search | Vector indexes                      | Structured data           |
| **WebSocket/SSE** | NestJS Gateway          | Real-time push notifications              | Live alerts, to-do updates          | Persistent state          |

<!-- FIXED: ClickHouse was missing from diagram in original — now added as explicit node -->

<!-- ADDED: WebSocket/SSE layer — Competitor Alerts + real-time notifications require it -->

<!-- ADDED: Transcription Service as explicit node — was missing from original diagram -->
...
```typescript
// ✅ CORRECT: AI work → delegate to AI Services
const summary = await aiServicesClient.post('/summarize', { transcript });
...
```typescript
// ✅ CORRECT: Competitor alert → WebSocket push
@WebSocketGateway()
export class AlertsGateway {
  @SubscribeMessage('competitor.detected')
  handleAlert(client: Socket, payload: AlertPayload) {
    client.emit('alert', payload); // Push to connected user
  }
}
...
> **New data store rule:** Adding ANY new data store (e.g., MongoDB, Cassandra, DynamoDB)
> **requires an ADR** (Section 4.7) with Tech Lead approval before a single line of
> integration code is written.

### 5.3 Layer-by-Layer Description
...
> // useEffect(() => fetchDeals().then(setDeals), []);
>
> // ❌ WRONG: Business logic in frontend
> // const riskScore = calculateDealRisk(deal); // Belongs in API layer
> ```

**What the frontend renders:**
Every user-facing screen — Deal Boards, Account Boards, Forecast Boards, Revenue
Dashboards, Sales Coaching Insights, AI Trainer, Email Composer, Engage To-do List,
Searchable Conversation Library, Ask Anything GenAI Query, and all admin config screens.

**How it connects to backend:**

- **REST APIs → API Layer** (all data reads/writes)
- **WebSocket/SSE → API Layer** (real-time: Competitor Alerts, live notifications)
- **Never** directly to DB, AI Services, or external systems

<!-- ADDED: WebSocket/SSE connection documented — was missing from original -->
...
| **Technology**    | **Role**                                                | **What It Does NOT Do**                 |
| ----------------------- | ------------------------------------------------------------- | --------------------------------------------- |
| **NestJS v11**    | Module system (maps 1:1 to M-01→M-10), DI, guards, pipes     | No AI inference, no direct warehouse access   |
...
| **Zod**           | Runtime validation of ALL API request bodies + event payloads | No unvalidated request reaches business logic |
...
<!-- ADDED: "What It Does NOT Do" column — enforces boundaries from the tech table itself -->

**Inter-Module Communication (Enforced Pattern):**

```typescript
// ✅ CORRECT: Module A → BullMQ event → Module B
// M-01 publishes
...
<!-- ADDED: Idempotency check in subscriber — critical gap from review -->

**API Layer → AI Services Layer Pattern:**

```typescript
// ✅ CORRECT: Sync (on-demand, user-triggered)
// M-06 calls AI Services for immediate summary
const summary = await this.aiServicesClient.post('/v1/summarize', {
  ...
  timeout: 30_000,           // ← Circuit breaker timeout
  signal: AbortSignal.timeout(30_000)
});

// ✅ CORRECT: Async (batch, background processing)
// M-04 enqueues theme analysis for all calls last 24h
await this.aiQueue.add('theme-analysis', {
  tenant_id: tenantId,
  call_ids: recentCallIds
}, {
  attempts: 3,               // ← Retry policy
  backoff: { type: 'exponential', delay: 5000 }
});

// ❌ WRONG: Sync call for batch operation (blocks request)
// const themes = await aiServicesClient.post('/detect-themes', { allCalls });
```

<!-- ADDED: Circuit breaker timeout + retry policy — critical gap from review -->

> **🚨 CIRCUIT BREAKER RULE:**
>
> Every synchronous call from API Layer → AI Services Layer MUST have:
>
> - **Timeout:** 30s max (user-facing), 5min (background)
> - **Retry:** 3 attempts with exponential backoff
> - **Fallback:** Return cached response or `null` (never throw 500 to user)
>
> ```typescript
> // Circuit breaker config (shared across all AI service calls)
> export const AI_CIRCUIT_BREAKER = {
>   timeout: 30_000,
>   errorThresholdPercentage: 50,
>   resetTimeout: 30_000,
>   fallback: () => ({ summary: null, cached: true })
> };
> ```

#### Layer 3 — AI Services Layer (Microservice)

**Technology:** FastAPI + Python 3.12 + LiteLLM + LangGraph + pyannote + spaCy
...
| **Python 3.12** | Runtime for all AI/ML (PyTorch, HuggingFace, LangChain, spaCy)                                                      | No product feature logic                        |
| **LiteLLM**     | Unified LLM gateway → OpenAI (primary) + Anthropic (fallback). Retries, token counting, cost tracking, rate limits | **API Layer never calls OpenAI directly** |
| **LangGraph**   | Stateful multi-step agent workflows (Deep Researcher, Ask Anything RAG, AI Trainer)                                 | No CRM writes, no DB access                     |
| **Whisper**     | Speech-to-text (Transcription Service — not AI Services Layer)                                                     | Not called from this service                    |
| **pyannote**    | Speaker diarization (Transcription Service)                                                                         | Not called from this service                    |
| **spaCy**       | NLP preprocessing — tokenization, entity extraction                                                                | No LLM calls                                    |

<!-- FIXED: Whisper + pyannote belong to Transcription Service, NOT AI Services Layer.
     Original incorrectly listed Whisper here. Clarified with explicit "Not called from 
     this service" note. -->

**Endpoints Exposed by the AI Services Layer:**

| **Endpoint**           | **Version** | **Called By**     | **Mode**  | **What It Does**                             | **Output**                                                           |
| ---------------------------- | ----------------- | ----------------------- | --------------- | -------------------------------------------------- | -------------------------------------------------------------------------- |
| `POST /v1/summarize`       | v1                | M-06 Insight Generation | **Sync**  | Call summaries, key points, next steps             | `{ summary, key_points[], next_steps[], confidence_score, tokens_used }` |
| `POST /v1/detect-trackers` | v1                | M-05 Smart Tracking     | **Async** | Intent-based signal detection vs tracker questions | `{ detections[], signal_strength, confidence }`                          |
| `POST /v1/generate-email`  | v1                | M-02 Sales Engagement   | **Sync**  | Personalized email draft from call context + CRM   | `{ subject, body, tone_score }`                                          |
| `POST /v1/answer-query`    | v1                | M-06 Ask Anything       | **Sync**  | RAG pipeline — retrieve + generate NL answer      | `{ answer, sources[], confidence_score }`                                |
| `POST /v1/score-call`      | v1                | M-04 Conv. Intelligence | **Async** | Score transcript against scorecard criteria        | `{ scores{}, overall_score, gaps[] }`                                    |
| `POST /v1/simulate-turn`   | v1                | M-10 AI Trainer         | **Sync**  | Next AI persona response in training simulation    | `{ persona_response, coaching_note }`                                    |
| `POST /v1/detect-themes`   | v1                | M-04 Theme Spotter      | **Async** | Topic modeling + cluster naming across call set    | `{ themes[], call_distribution }`                                        |
| `POST /v1/embed`           | v1                | M-05, M-06              | **Async** | Generate vector embeddings for semantic search     | `{ embedding[], model_used }`                                            |

<!-- ADDED: Version prefix (/v1/) — internal API versioning prevents deploy coordination
     issues when schema evolves. ADDED: Output schemas for every endpoint. -->

> **🚨 ENDPOINT VERSIONING RULE:**
>
> Every endpoint is versioned (`/v1/`, `/v2/`). When output schema changes:
>
> 1. **Create `/v2/` endpoint** — do NOT modify `/v1/`
> 2. **Deprecate `/v1/`** — notify all callers
> 3. **Old version stays live for 30 days** minimum
>
> ```python
> # ✅ CORRECT: Versioned router
> v1_router = APIRouter(prefix="/v1")
> v2_router = APIRouter(prefix="/v2")
>
> @v1_router.post("/summarize")
> async def summarize_v1(req: SummarizeRequestV1): ...
>
> @v2_router.post("/summarize")  # New schema with language_code
> async def summarize_v2(req: SummarizeRequestV2): ...
>
> # ❌ WRONG: Modify existing endpoint schema
> # @v1_router.post("/summarize")  # Adding new required field breaks callers
> ```

> **AI Hallucination / Quality Gate (Mandatory):**
>
> ```python
> class SummarizeResponse(BaseModel):
>     summary: str
...
>
> # Every endpoint enforces confidence threshold
> if response.confidence_score < 0.7:
>     response.flagged_for_review = True
>     # API Layer does NOT auto-write to CRM — flags for human review
> ```

<!-- ADDED: Confidence threshold + hallucination gate — critical gap from review -->

---

#### Layer 4 — Data Layer

The Data Layer is the set of **storage systems the API Layer reads from and writes to**.
Different stores are chosen for different data characteristics — each has a single,
specific job.

| **System**       | **Technology**             | **What It Stores**                                                                                    | **Why This Store**                            | **Scale Ceiling**           | **Phase Status** |
| ---------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | --------------------------------- | ---------------------- |
| **Primary DB**   | Supabase PostgreSQL (RLS + PITR) | Users, accounts, contacts, deals, transcripts, detections, scorecard results, email records, CRM sync state | ACID + RLS + multi-tenant isolation                 | ~500GB per tenant                 | ✅**Phase 1**    |
| **Cache**        | Redis (Upstash HA Phase 2)       | Sessions, rate limit counters, presence, short-lived results                                                | <1ms retrieval, TTL-based                           | N/A (ephemeral)                   | ✅**Phase 1**    |
| **Event Bus**    | BullMQ on Redis                  | Transcription jobs, AI processing, export jobs, email scheduling, inter-module events                       | Durable jobs, retries, dead-letter, priority queues | ~10k jobs/min                     | ✅**Phase 1**    |
| **Search Index** | Meilisearch                      | Call transcripts, emails, account/deal metadata (100+ filter params)                                        | Sub-50ms typo-tolerant full-text search             | ~50M documents                    | ✅**Phase 1**    |
| **Analytics**    | ClickHouse                       | Revenue metrics, coaching metrics, activity streams, dashboards                                             | Columnar, fast aggregations, billions of events     | **Unlimited (append-only)** | 🟡**Phase 2**    |
| **Vector Store** | pgvector (PostgreSQL extension)  | Embeddings of transcripts, emails, deal data for RAG + semantic search                                      | Co-located with source data, no separate DB         | **~1M vectors** ⚠️        | ✅**Phase 1**    |

<!-- ADDED: Scale Ceiling column — critical for capacity planning -->

> **⚠️ pgvector Scale Warning:**
>
> pgvector performs well up to ~1M embeddings. Beyond that, query latency degrades.
> For large enterprise tenants (1000+ calls/month), this ceiling can be reached within
> 12 months.
>
> **Phase 3 Migration Trigger:** When any tenant's vector count exceeds 800k:
>
> - Evaluate dedicated vector DB (Weaviate, Qdrant, Pinecone)
> - Raise ADR before migration
> - Existing pgvector data migrated via idempotent ETL job

<!-- ADDED: pgvector scale ceiling warning + migration trigger — critical gap from review -->

> **Data Store Selection Rules:**
>
> | **Data Type**          | **Use This Store** | **Not This**                          |
> | ---------------------------- | ------------------------ | ------------------------------------------- |
> | Transactional (deals, calls) | PostgreSQL               | ❌ Redis, ClickHouse                        |
> | Search (100+ filters)        | Meilisearch              | ❌ PostgreSQL LIKE queries                  |
> | Dashboards + metrics         | ClickHouse               | ❌ PostgreSQL aggregations                  |
> | RAG embeddings               | pgvector                 | ❌ Standalone vector DB (until ceiling hit) |
> | Queue + sessions             | Redis                    | ❌ PostgreSQL job tables                    |
>
> **Adding a new store requires an ADR (Section 4.7) before any integration code.**

**Database Indexing Requirements (Mandatory):**
...
<!-- ADDED: Mandatory indexing strategy — #1 cause of PostgreSQL production slowness -->

#### Layer 5 — Infrastructure

**Technology:** Docker + GitHub Actions + Railway + Sentry + Grafana + Better Stack +
Cloudflare + Doppler

The Infrastructure Layer is the platform all services run on and the tooling that
**operates, monitors, and secures** them.

| **Technology**     | **Role**                                                                                                                            | **Phase 3 Upgrade**                        |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| **Docker**         | All services run as containers. Every service has a `Dockerfile`. Local dev uses Docker Compose for full stack                          | AWS ECS Fargate                                  |
| **GitHub Actions** | CI/CD: PR → type checks + tests + Zod + Prisma validation. Merge → build image → push → Railway deploy                                | Same pipeline, new target                        |
| **Railway**        | Managed container hosting — orchestration, health checks, restarts, env injection. Chosen for Phase 1 to eliminate Kubernetes overhead   | **AWS ECS** (Phase 3 trigger: 500 tenants) |
| **Sentry**         | Error tracking (Next.js + NestJS + Python). Full stack traces + user context + request metadata. Every unhandled exception = Sentry issue | Same                                             |
| **Grafana**        | Metrics dashboards — response times, queue depths, DB pool usage, cache hit rates, AI latency, error rates                               | Same + distributed tracing                       |
| **Better Stack**   | Log aggregation (all services, real-time). Structured search, log pattern alerts, uptime monitoring                                       | Same                                             |
| **Cloudflare**     | CDN (static assets), DDoS protection, TLS termination, WAF, rate limiting (outer layer)                                                   | Same                                             |
| **Doppler**        | Secrets management — ALL env vars (DB strings, API keys, OAuth creds, LLM keys). No secrets in repos or images                           | Same                                             |

<!-- ADDED: "Phase 3 Upgrade" column — makes migration path explicit per technology -->

> **🚨 Phase 3 Infrastructure Migration Triggers:**
>
> | **Trigger**                         | **Metric**                    | **Action**              |
> | ----------------------------------------- | ----------------------------------- | ----------------------------- |
> | **Railway → AWS ECS**              | >500 tenants OR >50M requests/month | Raise ADR-024-v2              |
> | **Redis → Upstash HA**             | Any Redis downtime incident         | Upgrade immediately (Phase 2) |
> | **pgvector → Qdrant**              | Any tenant >800k vectors            | Raise ADR-021-v2              |
> | **ClickHouse Cloud → Self-hosted** | >$5k/month ClickHouse bill          | Raise ADR-020-v2              |

**CI/CD Pipeline (GitHub Actions — Mandatory Gates):**

```yaml
# Every PR must pass ALL before merge
pr-checks:
  steps:
    ...
    - name: RLS enforcement check   # ← ADDED: critical security gate
      run: ts-node scripts/check-rls-all-tables.ts

    - name: No cross-module imports  # ← ADDED: enforces Principle 1
...
<!-- ADDED: RLS enforcement check + module boundary check as CI gates -->

**Observability Stack (All Must Be Active):**

```typescript
// Sentry — Every unhandled exception
Sentry.init({
...
// Grafana — Key metrics to alert on
const GRAFANA_ALERTS = [
  { metric: 'bullmq_queue_depth', threshold: 1000, severity: 'critical' },
  { metric: 'ai_service_latency_p99', threshold: 5000, severity: 'warning' },
  { metric: 'postgres_connections', threshold: 80, severity: 'warning' },
  { metric: 'redis_memory_usage', threshold: 85, severity: 'critical' },
  { metric: 'error_rate_5xx', threshold: 1, severity: 'critical' }  // >1% = alert
];
```

---

### 5.4 How a Request Flows Through the Architecture
...
sequenceDiagram
  ...
  API->>API: JWT Guard → verify token + Manager role
  API->>API: Zod → validate query params
  API->>PG: Prisma → read deals + AI risk signals
  Note over API,PG: AI risk signals already computed async<br/>when call.transcription.completed fired
  PG-->>API: Deal records + pre-computed risk scores
  API-->>FE: JSON response <500ms
...
### 5.5 Technology Decision Summary
...
| LLM routing               | LiteLLM → OpenAI API         | ADR-006       | ✅ Approved      |
...
<!-- ADDED: Status column — every ADR must be Approved before tech enters codebase -->
...
are the ground truth for how data moves through Relanto Revenue Intelligence.
...
14. Publish `call.transcription.completed` event to BullMQ (payload: `event_id`, `call_id`, `transcript_id`, `tenant_id`, `duration_seconds`, `language_detected`, `confidence_score`).
...
| Step 7 — Whisper transcription | Model timeout or error     | Retry once on Whisper; fall back to AssemblyAI; if both fail → status =`transcription_failed` + Sentry alert |
...
2. Fetch deal and account context from M-03 Revenue Graph:
   ...
   - If no deal is linked yet (M-03 entity linking still in progress), wait up to **5 minutes** using BullMQ delayed job before proceeding without context. See M-04 Processing Order Rule (Section 6.6).
...
| Step 4 — AI Services timeout | LLM or network failure           | Retry × 3; after 3 failures → skip summary, mark call record `summary_failed`, Sentry alert |
| Step 6 — DB write failure    | PostgreSQL unavailable           | BullMQ job retries × 3; dead-letter queue on exhaustion                                        |
| Step 8 — CRM write failure   | CRM API rate limit or auth error | Retry × 3 with exponential backoff; log to `crm_sync_logs` with `error_message`            |

> **Idempotency Rule:**
> If `call.transcription.completed` is received twice for the same `call_id`
> (duplicate event), M-06 checks for an existing `call_summaries` record before
> processing. If `version = 1` already exists, the job is skipped.
> Re-generation is only triggered by an explicit user action (future endpoint:
...
7. Write results to PostgreSQL:
   ...
   SET health_score    = :computed_score,
       health_category = :category,         -- healthy | at_risk | critical
       last_signal_at  = NOW()
   WHERE deal_id = :deal_id
     AND tenant_id = :tenant_id;
   ```
...
| **Step**             | **Failure**                  | **Recovery**                                                    |
| -------------------------- | ---------------------------------- | --------------------------------------------------------------------- |
| Step 1–2 — DB read       | PostgreSQL unavailable             | BullMQ job retries × 3 with backoff; deal health score remains stale |
...
```mermaid
flowchart TB
  EVT["BullMQ: call.transcription.completed"] --> M06["M-06 Insight Generation (NestJS)"]
  M06 --> JOB["BullMQ job: post_call_agent.run\n(call_id, transcript_id, tenant_id, deal_id, account_id, contact_ids[])"]
  JOB --> AIS["AI Services Layer (FastAPI)"]

  subgraph AGENT["PostCallAgent — LangGraph stateful workflow"]
    direction TB
  ...
  N2 -->|tracker_detections[]| M05["M-05 Smart Tracking (stores detections)"]
  N3 -->|call_summary| M06
  N4 -->|call_score| M04["M-04 Conversation Intelligence (stores scores)"]
  N5 -->|email_draft| M02["M-02 Sales Engagement (creates draft)"]
  N6 -->|competitor match| M08["M-08 Execution & Automation (sends alert)"]
```
...
| **Node 2** — Detect trackers  | Transcript + published tracker definitions  | `POST /v1/detect-trackers`                          | `tracker_detections[]` → M-05 `tracker_detections` table |
| **Node 3** — Generate summary | Transcript + deal context + N2 detections   | `POST /v1/summarize`                                | `call_summary` → M-06 `call_summaries` table             |
| **Node 4** — Score call       | Transcript + active scorecards              | `POST /v1/score-call`                               | `call_score` → M-04 `call_scores` table                  |
| **Node 5** — Email draft      | N3 next_steps + deal context                | `POST /v1/generate-email`                           | `email_draft` → M-02 `email_drafts` table                |
| **Node 6** — Competitor check | N2 detections +`competitor_alert_configs` | In-memory match — no LLM call                        | Triggers M-08 alert if match + confidence ≥ threshold        |
| **Node 7** — Audit log        | All node outputs                            | Write `post_call_run_id`, node statuses, durations  | `post_call_agent_runs` audit table                          |
...
tracker detections from Node 2 to highlight the most significant signals in the
summary. Node 5 (email draft) uses the next steps from Node 3 to write the email
body. LangGraph's stateful graph maintains a shared state object across all nodes
— no node re-fetches data that a previous node already loaded. This also means a
single context window is built once in Node 1 and passed through — not rebuilt at
each step.
...
| ------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Any node — LLM timeout         | LiteLLM call exceeds timeout     | Retry that node × 2; skip node + log if still failing. Subsequent nodes continue with partial state. |
...
> The platform never leaves a call with zero outputs — Node 3 (summary)
> is the only node with no fallback skip; it retries indefinitely until
> the BullMQ job TTL expires (24 hours).

---

### 7.6 Flow 5 — CRM Sync (Relanto RI → Salesforce / HubSpot / Dynamics)
...
flowchart TB
  ...
  subgraph FULL["Trigger A — Scheduled Daily Full Sync (02:00 UTC)"]
    direction TB
    A1["Enqueue crm.sync.full per active tenant"]
  ...
  subgraph PARTIAL["Trigger B — Real-time Activity Sync (call.summary.generated)"]
    direction TB
    B1["Fetch call summary from call_summaries"]
    B2["Fetch crm_deal_id / crm_account_id / crm_contact_id\nfrom linked deal + contact records"]
    B3["Create Activity record on CRM Opportunity\n(type='Call', body=one_line_summary + key_points + next_steps)"]
...
| Trigger A — Partial sync failure     | One entity type fails mid-sync | Log failed entity type to `crm_sync_logs` with `status='partial'`; retry that entity on next cron |
...
| Trigger B — No CRM ID linked         | Deal not yet synced from CRM   | Skip CRM write; set `pushed_to_crm = false` permanently until next full sync links the entity       |

> **🚨 CRM Write Boundary (Section 3.7):**
> M-03 writes **only** to:
>
> - Activity records (call notes)
> - AI-extracted field values on Opportunities
>
> M-03 **never** writes to:
>
> - Deal stage (Opportunity stage is the CRM's source of truth — M-03 only reads it)
> - Contact or Account owner fields
> - Any field not explicitly in `crm_extracted_fields`
...
> **🚨 Token Storage Rule (Section 3.7):**
> `access_token` must be stored in memory (a module-level variable or React context).
> Never in `localStorage` or `sessionStorage`. XSS attacks cannot steal what JS
> cannot read. The `refresh_token` in an HttpOnly cookie is equally protected.
...
- Runs immediately after JWT Guard
- Looks up `tenant_id`, `user_id`, `role` from `users` table using `supabase_user_id`
...
- Reads `@Roles()` decorator on the route handler
- Compares `req.context.role` against required roles
...
> **🚨 Prisma Middleware Rule (Platform Boundary — Section 3.7):**
> This middleware is the **last line of defense** for org isolation.
> Even if a developer forgets `WHERE tenant_id = x` in a query, the middleware
> adds it automatically. It must **never** be disabled, bypassed, or overridden.
...
| **Layer 4** — Route       | RBAC Guard +`@Roles()` decorator                    | Role escalation — reps accessing manager or admin data |
...
| ------------------------------------------------ | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **Flow 1** — Call Ingestion to Transcript | Webhook: Zoom / Teams / Meet / Dialer call ended                                      | Webhook → HMAC validation → BullMQ → Transcription Service → Whisper → Transcript stored →`call.transcription.completed` emitted | 5 minutes for a 60-minute call             |
| **Flow 2** — AI Summary Generation        | `call.transcription.completed` event                                                | M-06 → PostgreSQL (transcript) → M-03 (context) → AI Services Layer → LiteLLM → Summary stored →`call.summary.generated` emitted | 30 seconds                                 |
...
| **Flow 4** — Post-Call Agentic Flow       | `call.transcription.completed` (async batch)                                        | M-06 → BullMQ → AI Services Layer → LangGraph (7 nodes) → all outputs written back to M-02, M-04, M-05, M-08                         | 90 seconds for a 60-minute call            |
| **Flow 5** — CRM Sync                     | Daily cron at 02:00 UTC (full) or `call.summary.generated` (partial)                | M-03 → CRM API → PostgreSQL upsert + stage change detection (full) OR summary → CRM activity write + field push (partial)             | Full: 10 min/tenant · Partial: 10 seconds |
...
asynchronous communication in Relanto Revenue Intelligence.

Every event that any module emits or consumes must be registered in Section 9.3.
If an event is not in this registry, it does not exist officially. Before emitting
a new event, add it to this registry first, get it reviewed, then implement it.

---

### 9.2 Event Bus Technology — BullMQ on Redis

Relanto Revenue Intelligence uses **BullMQ** as its event bus and job queue system,
backed by **Redis 7.x**.
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Durable job queues   | Jobs survive process restarts — Redis persists queue state. A job enqueued before a deployment is still there when the service comes back up |
| Retry with backoff   | Each queue has a configured retry count and backoff strategy. Failed jobs are automatically retried without developer intervention            |
| Dead letter queues   | Jobs that exhaust all retries are moved to a dead letter queue (DLQ) for manual inspection and replay                                         |
| Job priorities       | High-priority jobs (e.g., real-time tracker detections) are processed before lower-priority batch jobs (e.g., Data Cloud exports)             |
| Delayed jobs         | Jobs can be scheduled with a delay — used by M-02 email scheduling and M-08 workflow automation step delays                                  |
| Concurrency control  | Each worker specifies its concurrency — how many jobs it processes simultaneously. Prevents resource exhaustion on AI workers                |
| Job events           | BullMQ emits events on job state changes (active, completed, failed, stalled) — consumed by the monitoring layer                             |

**Why BullMQ over other options:**

| **Alternative**             | **Why Not Chosen**                                                                                                        |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Direct HTTP calls between modules | Creates tight coupling — if the receiving module is down, the call fails immediately with no retry                             |
| Kafka                             | Operationally complex for a 25-person team. Requires Zookeeper or KRaft cluster management. Overkill for Phase 1 event volumes  |
| RabbitMQ                          | Requires separate infrastructure from Redis. BullMQ reuses the Redis instance already needed for caching and session management |
| AWS SQS                           | Cloud vendor lock-in. Railway hosting works better with self-contained infrastructure                                           |
| PostgreSQL LISTEN/NOTIFY          | No retry, no backoff, no dead letter queue, no job priority — insufficient for production event processing                     |

---

### 9.3 Full Event Registry

Every event in the platform is documented below. Format for each event:

- **Queue name** — the BullMQ queue the event is published to
- **Producer** — the module that publishes the event
- **Consumers** — the modules that subscribe to the event
- **Payload** — the exact JSON fields in the job data
- **Retry policy** — retry count, backoff strategy, and DLQ name on exhaustion

> **Registry rule:** Before emitting a new event, add it to this registry,
> get the entry reviewed, then implement it. A PR that adds a new BullMQ
> `queue.add()` call without a corresponding entry in this registry will
> not be merged.
...
```json
{
  ...
  "speakerCount":    "number",
  "participantList": [
    { "name": "string", "email": "string", "speakerLabel": "string" }
  ],
  "languageDetected":  "string (BCP 47 language code)",
...
| **Consumer**             | **Action**                                                                                                                                                   |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| M-02 Sales Engagement          | Creates an AI-suggested follow-up email task for the call owner in `engagement.tasks`. Idempotency guard: skip if task already exists for `source_id = callId` |
| M-03 Revenue Graph             | Links the call to the correct account, deal, and contact using participant email matching. Creates an `accounts.activities` record                               |
| M-04 Conversation Intelligence | Enqueues async jobs for call scoring and topic tagging                                                                                                             |
...
**Consumer action:** M-03 writes AI-extracted field values to `accounts.deals` /
`accounts.contacts` and pushes them to the connected CRM via the Salesforce /
HubSpot / Dynamics API. Sets `pushed_to_crm = true` on
`transcription.crm_extracted_fields` on success.
...
| Consumers       | M-03 Revenue Graph, M-05 Smart Tracking, M-07 Deal & Account Mgmt |
| Priority        | **Normal (2)**                                              |
| DLQ             | `email.sent.dlq`                                                |
...
| **Consumer**       | **Action**                                                                               |
| ------------------------ | ---------------------------------------------------------------------------------------------- |
| M-03 Revenue Graph       | Logs the email as an activity record on the linked deal and contact in `accounts.activities` |
| M-05 Smart Tracking      | Runs tracker detection on the email body for competitor and intent signal detection            |
| M-07 Deal & Account Mgmt | Updates `last_activity_at` on the linked deal record                                         |
...
> **`matchConfidence = "unmatched"` rule:** M-04 and M-05 must still process
> the call — conversation intelligence and tracker detection run regardless of
> CRM linkage status. The `unmatched` status is surfaced in the UI as a warning
> prompting the rep to manually link the call to a deal or account.
...
| Producer        | M-03 Revenue Graph                                                      |
| Consumers       | M-07 Deal & Account Mgmt, M-08 Execution & Automation, M-09 Forecasting |
| Priority        | **High (1)**                                                      |
...
| **Consumer**          | **Action**                                                                                                           |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| M-07 Deal & Account Mgmt    | Recalculates deal health score. Updates deal position on the Deals Board                                                   |
| M-08 Execution & Automation | Evaluates workflow automation rules triggered by stage entry. Enrolls deal in configured plays if trigger conditions match |
...
| **Consumer**      | **Action**                                                                                      |
| ----------------------- | ----------------------------------------------------------------------------------------------------- |
| M-05 Smart Tracking     | Uses topic context to improve tracker detection ranking — topic-matched detections are scored higher |
...
| Producer        | M-04 Conversation Intelligence |
| Consumers       | M-10 Performance & Coaching    |
| Priority        | **Normal (2)**           |
| DLQ             | `call.scored.dlq`            |
...
```json
{
  ...
  "talkRatio":               "number (0–1, rep's share of talk time)",
...
**Consumer action:** M-10 writes the score to the ClickHouse `call_score_events`
table and updates the rep's `coaching.coaching_snapshots` for the current period.
If `flaggedForReview = true`, M-10 creates a low-priority coaching recommendation
for the rep's manager.
...
| Consumers       | M-06 Insight Generation, M-07 Deal & Account Mgmt, M-08 Execution & Automation |
| Priority        | **High (1)**                                                             |
...
| M-07 Deal & Account Mgmt    | Creates or updates a `deals.deal_risk_flag` if `trackerType` is `competitor`, `objection`, or `pricing`. Recalculates deal health score                 |
| M-08 Execution & Automation | Evaluates configured competitor alert rules. If matched: sends Slack notification and in-app alert. Evaluates workflow automation rules triggered by tracker type |
...
| Consumers       | M-03 Revenue Graph, M-07 Deal & Account Mgmt |
| Priority        | **Normal (2)**                         |
...
| **Consumer**       | **Action**                                                                                                   |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| M-03 Revenue Graph       | Formats summary as a CRM activity note and writes it to the linked opportunity in Salesforce, HubSpot, or Dynamics |
| M-07 Deal & Account Mgmt | Triggers a deal brief refresh for the linked deal. Updates `last_signal_at` on the deal record                   |
...
**Consumer action:** M-10 updates the rep's forecast submission history on the
Revenue Dashboard. If this is the first submission for the period, M-10 triggers
an AI forecast snapshot computation in M-09.

---

### 9.4 Event Priority Reference
...
### 9.5 Dead Letter Queue Strategy

Every queue has a corresponding DLQ named `<queue-name>.dlq`. A job is moved
to its DLQ after all configured retries are exhausted.
...
4. If the failure was transient (external API timeout, network partition): replay
   the job directly from Bull Board
5. If the failure was a data bug (malformed payload, missing FK): fix the data,
   then replay the job

**DLQ replay procedure (TypeScript):**

```typescript
// Replay a single failed job from DLQ
...
### 9.6 Queue Naming Conventions

All BullMQ queue names in Relanto Revenue Intelligence follow these rules:

1. **Format:** `<domain>.<event>` — dot-separated, all lowercase, no underscores
2. **DLQ suffix:** Append `.dlq` to the original queue name — never use a separate naming pattern
3. **Internal job queues** (not cross-module events): prefix with the module domain
   - Examples: `ingestion.process-audio`, `conversation.score-call`, `engagement.send-email-step`
4. **No version suffixes** in queue names at this stage — versioning is handled via
   payload schema evolution with backward-compatible additions only

| **Pattern**                     | **Example**                    | **Use For**            |
| ------------------------------------- | ------------------------------------ | ---------------------------- |
| `<domain>.<noun>.<past-tense-verb>` | `call.transcription.completed`     | Cross-module platform events |
| `<domain>.<verb>-<noun>`            | `conversation.score-call`          | Internal module job queues   |
| `<original-queue>.dlq`              | `call.transcription.completed.dlq` | Dead letter queues           |
...
### 9.5 Job Queue Naming Convention

All BullMQ queues follow a strict naming convention. Consistent queue names make
monitoring, alerting, and dead letter queue management predictable.

**Convention: `[domain].[entity].[action]`**

| **Segment** | **Rule**                                           | **Examples**                                                                                             |
| ----------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `domain`        | Lowercase module domain name                             | `transcription`, `conversation`, `deal`, `account`, `email`, `tracker`, `forecast`, `coaching` |
| `entity`        | The primary entity the job operates on                   | `call`, `transcript`, `deal`, `tracker`, `summary`, `email`, `forecast`                          |
| `action`        | Past tense for events; imperative/command for job queues | `completed`, `extracted`, `changed` / `process`, `sync`, `export`                                  |

**Event queues** (things that happened — past tense):
...
**Command queues** (jobs to be processed — imperative):

transcription.process -- Process a new audio file through Whisper
post_call_agent.run -- Run the full PostCallAgent LangGraph workflow
tracker.detect -- Run tracker detection on a transcript or email
summary.generate -- Generate a call summary
deal_brief.refresh -- Refresh the AI deal brief for a deal
crm.sync.full -- Full CRM sync for a tenant
crm.sync.activity -- Sync a single activity back to CRM
data_cloud.export -- Run a Data Cloud warehouse export for a tenant
email.schedule -- Send a scheduled email at the configured time
embedding.generate -- Generate and store vector embeddings for an entity
analytics.write -- Write an event row to ClickHouse

**Dead letter queues** (failed jobs after all retries exhausted):
[original-queue-name].dlq
...
> If a new queue is needed, propose the name in the ADR or tech design review
> before implementing. A PR that introduces a new `new Queue(...)` call with a
> non-conforming name will not be merged.

---

### 9.6 BullMQ Worker Configuration Reference

Every BullMQ worker in the platform declares its concurrency and queue options
explicitly. This table documents the standard settings per queue category.

| **Queue Category**                                                                    | **Concurrency** | **Limiter**            | **StalledInterval** | **Rationale**                                                                             |
| ------------------------------------------------------------------------------------------- | --------------------- | ---------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------- |
| AI-heavy command queues (`post_call_agent.run`, `summary.generate`, `tracker.detect`) | 3                     | 3 jobs / 1 second per worker | 60s                       | AI service calls are expensive and slow — low concurrency prevents overloading FastAPI workers |
| Event queues (all cross-module events)                                                      | 10                    | None                         | 30s                       | Fast DB writes only — higher concurrency is safe                                               |
| CRM sync queues (`crm.sync.full`, `crm.sync.activity`)                                  | 2                     | 5 jobs / 10 seconds          | 120s                      | CRM API rate limits (Salesforce: 100 req/min per OAuth token)                                   |
| Embedding queues (`embedding.generate`)                                                   | 5                     | 10 jobs / 1 second           | 60s                       | OpenAI Embeddings API allows higher throughput than chat completions                            |
| Analytics write queues (`analytics.write`)                                                | 20                    | None                         | 30s                       | ClickHouse batch inserts are lightweight — high concurrency maximises throughput               |
| Export queues (`data_cloud.export`)                                                       | 1                     | None                         | 300s                      | One export per tenant at a time — idempotency key prevents duplicates                          |

**Standard worker declaration pattern:**

```typescript
// Standard BullMQ worker registration in NestJS
@Injectable()
export class CallTranscriptionWorker {
  ...
  static register(): WorkerOptions {
    return {
      ...
      removeOnComplete: { count: 1_000 },   // retain last 1000 completed jobs
...
### 9.6 Dead Letter Queue Strategy

A dead letter queue (DLQ) is where BullMQ moves a job after it has failed all
configured retries. A job in the DLQ is not lost — it is held indefinitely until
a developer or an automated process takes action on it.

**When a job goes to the DLQ:**
A job moves to the DLQ when it has failed its maximum configured retry count and
the final retry has also failed. For example, a `transcription.process` job with
3 retries and exponential backoff will be attempted 4 times total (1 initial +
3 retries) before landing in the DLQ.

---

**DLQ handling by severity:**

| **Queue**                      | **Severity** | **Automated Alert**           | **Manual Action Required**                                             |
| ------------------------------------ | ------------------ | ----------------------------------- | ---------------------------------------------------------------------------- |
| `call.transcription.completed.dlq` | 🔴 Critical        | Sentry alert + Slack `#incidents` | Inspect failure reason, fix root cause, replay job                           |
| `post_call_agent.run.dlq`          | 🔴 Critical        | Sentry alert + Slack `#incidents` | Replay after fixing AI Services Layer issue                                  |
| `transcription.process.dlq`        | 🔴 Critical        | Sentry alert + Slack `#incidents` | Check audio file availability, Whisper / AssemblyAI status                   |
| `crm.sync.full.dlq`                | 🟠 High            | Sentry alert + Slack `#ops`       | Check CRM API credentials and rate limits                                    |
| `tracker.detection.created.dlq`    | 🟠 High            | Sentry alert                        | Review AI Services Layer tracker detection endpoint                          |
| `call.summary.generated.dlq`       | 🟡 Medium          | Better Stack log alert              | Investigate and replay when AI Services Layer is healthy                     |
| `email.sent.dlq`                   | 🟡 Medium          | Better Stack log alert              | Check Gmail / Outlook API credentials. Notify rep if email was not delivered |
| `data_cloud.export.dlq`            | 🟡 Medium          | Better Stack log alert              | Check warehouse credentials. Retry manually via RevOps admin panel           |
| `forecast.submitted.dlq`           | 🟢 Low             | Logged to Better Stack only         | Review and replay during next business day                                   |

---

**How to replay a DLQ job:**

**Option 1 — Bull Board UI** *(recommended for individual jobs)*

1. Open Bull Board at the monitoring URL (see Section 9.7)
2. Navigate to the DLQ for the failed queue
3. Inspect the job data and failure reason
4. Click **Retry** to move the job back to the active queue

**Option 2 — Admin CLI** *(for bulk replay)*

```bash
# Moves all jobs from the DLQ back to the active queue in batches of 50
npm run queue:replay --queue=call.transcription.completed.dlq
```

**Option 3 — Automated replay for transient failures**

A scheduled BullMQ cron job runs every 60 minutes and automatically replays
jobs from low-severity DLQs (`forecast.submitted.dlq`, `email.sent.dlq`) where
the failure reason matches known transient errors (network timeout, rate limit 429).

> **Rule:** Jobs with application errors (`NullPointerException`, invalid payload,
> schema validation failure) are **never** auto-replayed. They require a developer
> fix before replay. Auto-replay is for infrastructure failures only.

```typescript
// Automated transient-error replay — runs every 60 minutes
@Cron('0 * * * *')
async replayTransientDlqJobs() {
  const transientPatterns = ['ECONNRESET', 'ETIMEDOUT', '429', 'rate limit'];
  const lowSeverityDlqs   = ['forecast.submitted.dlq', 'email.sent.dlq'];

  for (const dlqName of lowSeverityDlqs) {
    const dlq      = new Queue(dlqName, { connection: this.redis });
    const failed   = await dlq.getJobs(['failed']);

    for (const job of failed) {
      const isTransient = transientPatterns.some(p =>
        job.failedReason?.includes(p)
      );
      if (!isTransient) continue; // skip — needs developer fix

      await this.activeQueues[dlqName.replace('.dlq', '')].add(
        job.name,
        job.data,
...
Jobs in DLQs are retained for **7 days**. After 7 days, they are automatically
removed. Any job that cannot be replayed successfully within 7 days must be
investigated and its root cause fixed. Data loss from expired DLQ jobs must be
logged as an incident in the incident tracker.

---

### 9.7 Bull Board — Queue Monitoring Dashboard

**What Bull Board is:**
Bull Board is an open-source web UI for BullMQ that shows the real-time state of
all queues — active jobs, waiting jobs, completed jobs, failed jobs, and DLQ jobs.
It is the primary tool for monitoring queue health and debugging failed jobs.

**URLs:**

| **Environment** | **URL**                                       |
| --------------------- | --------------------------------------------------- |
| Production            | `https://queues.r-ri-internal.relanto.ai`         |
| Staging               | `https://queues.staging.r-ri-internal.relanto.ai` |
| Local dev             | `http://localhost:3001/queues`                    |

**Access:** Bull Board is only accessible on the internal network (not publicly
accessible). In production it is behind Cloudflare Access with Google SSO — only
Relanto.ai engineers and RevOps users with the correct Google Workspace account
can access it.

---

**What to monitor in Bull Board:**

| **Metric**                               | **Healthy State** | **Alert Threshold**                                       |
| ---------------------------------------------- | ----------------------- | --------------------------------------------------------------- |
| `transcription.process` — waiting job count | < 20 jobs               | > 100 jobs → Transcription Service may be down or under-scaled |
| `post_call_agent.run` — waiting job count   | < 50 jobs               | > 200 jobs → AI Services Layer backlog building                |
| Any queue — failed job count                  | 0                       | > 5 failures in 10 minutes → Sentry alert fires automatically  |
| Any critical DLQ — job count                  | 0                       | Any job in any Critical DLQ → Sentry alert fires automatically |
| `crm.sync.full` — last completed at         | Within last 26 hours    | > 26 hours since last success → CRM sync may have stopped      |
| `data_cloud.export` — last completed at     | Within last 26 hours    | > 26 hours → Data Cloud export may have stopped                |

---

**Bull Board setup in NestJS:**

```typescript
// main.ts — Bull Board mounted as Express middleware
...
createBullBoard({
  queues: [
    new BullMQAdapter(transcriptionProcessQueue),
    new BullMQAdapter(postCallAgentQueue),
    new BullMQAdapter(trackerDetectQueue),
    new BullMQAdapter(summaryGenerateQueue),
    new BullMQAdapter(dealBriefRefreshQueue),
    new BullMQAdapter(crmSyncFullQueue),
    new BullMQAdapter(crmSyncActivityQueue),
    new BullMQAdapter(dataCloudExportQueue),
    new BullMQAdapter(emailScheduleQueue),
    new BullMQAdapter(embeddingGenerateQueue),
    new BullMQAdapter(analyticsWriteQueue),
    // Event queues
    new BullMQAdapter(callTranscriptionCompletedQueue),
    new BullMQAdapter(trackerDetectionCreatedQueue),
    new BullMQAdapter(dealStageChangedQueue),
    new BullMQAdapter(emailSentQueue),
    new BullMQAdapter(forecastSubmittedQueue),
  ],
  serverAdapter,
});

// authMiddleware enforces Cloudflare Access JWT verification in production
app.use('/queues', authMiddleware, serverAdapter.getRouter());
```

### 9.8 Worker Concurrency Configuration

Each BullMQ worker declares how many jobs it processes simultaneously. This
prevents resource exhaustion — especially on the AI Services Layer where each
job makes one or more LLM API calls that consume significant memory, CPU, and
OpenAI token quota.

| **Queue**           | **Worker Location**       | **Concurrency** | **Reason**                                                                                                           |
| ------------------------- | ------------------------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `transcription.process` | Transcription Service (FastAPI) | 5                     | Audio processing is CPU and memory intensive. More than 5 simultaneous Whisper runs would exhaust container memory         |
| `post_call_agent.run`   | AI Services Layer (FastAPI)     | 3                     | LangGraph agent runs make multiple sequential LLM calls. 3 simultaneous agents balances throughput with OpenAI rate limits |
| `tracker.detect`        | AI Services Layer (FastAPI)     | 10                    | Tracker detection is faster than full agent runs. 10 concurrent is safe within OpenAI TPM limits                           |
| `summary.generate`      | AI Services Layer (FastAPI)     | 5                     | Single LLM call per job. 5 concurrent stays within OpenAI TPM limits                                                       |
| `embedding.generate`    | AI Services Layer (FastAPI)     | 10                    | Embedding API calls are fast and lightweight — higher concurrency is safe                                                 |
| `analytics.write`       | API Layer (NestJS)              | 20                    | ClickHouse writes are fast append-only inserts. High concurrency improves throughput                                       |
| `crm.sync.full`         | API Layer (NestJS)              | 1 per tenant          | One full sync per tenant at a time prevents CRM API rate limit exhaustion (Salesforce: 100 req/min per OAuth token)        |
| `email.schedule`        | API Layer (NestJS)              | 10                    | Email sends are I/O-bound — high concurrency is safe                                                                      |

> **Scaling rule:** Concurrency values are set for Phase 1–2 container sizing
> (2 vCPU / 4 GB RAM per worker container on Railway). If a worker container is
> scaled vertically, concurrency may be increased proportionally. Any change to
> concurrency values must be documented in the relevant module's ADR entry and
> tested against OpenAI rate limits in staging before deploying to production.

**Concurrency configuration pattern (NestJS):**

```typescript
// Worker registered with explicit concurrency in NestJS module
WorkerHost.register({
  name:    'post_call_agent.run',
  options: {
    concurrency:     3,
    stalledInterval: 60_000,
    maxStalledCount: 2,
    limiter: {
      max:      3,      // max 3 jobs active at once across all worker instances
      duration: 1_000,  // per second — prevents burst overload on AI Services Layer
    },
  },
})
```

**Concurrency configuration pattern (FastAPI / Python workers):**

```python
# worker.py — FastAPI BullMQ consumer via bullmq Python library
worker = Worker(
    'post_call_agent.run',
    process_job,
    redis_opts=RedisOpts(host=REDIS_HOST, port=REDIS_PORT),
    concurrency=3   # matches NestJS registration — must stay in sync
)
```

> **⚠️ NestJS ↔ FastAPI sync rule:** For queues where NestJS enqueues and
> FastAPI consumes (e.g., `transcription.process`, `post_call_agent.run`,
> `tracker.detect`, `summary.generate`, `embedding.generate`), the concurrency
> value must be kept in sync across both sides. A mismatch causes either
> under-utilisation or queue starvation. The canonical value is defined in
> `config/queue-concurrency.ts` and imported by both the NestJS producer
> registration and referenced in the FastAPI worker documentation.
