...
This document is the single working inventory of all tools, libraries, services, APIs, models, platforms, and external integrations used or planned for the development, testing, deployment, monitoring, and scaling of the R-Revenue Intelligence platform. It exists so that anyone on the team can quickly understand what we use, why we use it, whether it is free or paid, whether it works offline or online, and where it fits in the product lifecycle. It also helps prevent random tool sprawl, duplicate purchases, and unsupported engineering decisions. [file:1]

This document should be used by:
- **Backend engineers**, to know which frameworks, queues, databases, APIs, and supporting services are approved for server-side development and integrations. [file:1]
- **Frontend engineers**, to know which UI frameworks, state libraries, charts, testing tools, and hosting services are approved for product development. [file:1]
- **AI/ML engineers**, to know which LLM providers, local models, orchestration frameworks, transcription tools, vector tools, and evaluation utilities are approved for AI feature development. [file:1]
- **QA engineers**, to know which test frameworks, browser automation tools, load testing tools, mocks, and test environments are expected for validation. [file:1]
- **DevOps engineers**, to know which deployment, observability, secrets, CDN, and hosting tools are part of the approved stack. [file:1]
- **Tech leads and architects**, to review whether a proposed tool fits the approved architecture, cost model, reliability needs, and scaling plan. [file:1]
- **Product managers and founders**, to understand which tools are required for MVP, which are optional, and which may create future cost commitments. [file:1]
...
- a major architecture or infrastructure decision changes the stack, such as moving from Railway to AWS or from one LLM provider to another [file:1]
...
- **LM Studio** or **Ollama** are tools we can use to run local AI models during experimentation or fallback testing. [file:1][web:11][web:19]
...
- **Whisper**, **spaCy**, and **scikit-learn** are libraries used for speech, NLP, and ML-related development. [file:1]
...
- **Sentry**, **Better Stack**, and **Cloudflare** are managed services for monitoring, logging, uptime, CDN, and edge protection. [file:1]
...
- performance tuning
- enterprise compliance or deployment constraints [file:1]
...
- **OpenAI API**, **AssemblyAI**, and evaluation-only provider APIs such as **xAI / Grok routes** are model or AI service integrations. [file:1][web:8]
...
- they often require special testing and fallback handling [file:1]
...
- local Gemma-family models when hardware supports them [file:1][web:11][web:19][web:15]
...
- managed AI API credits or introductory usage [file:1]
...
- GPU infrastructure for heavy inference workloads [file:1]
...
- **Ollama** because it allows us to run open models locally for development and fallback experiments. [file:1][web:26]
...
- **Whisper self-hosted** because we need offline or self-controlled transcription for testing and for reducing dependence on third-party speech APIs. [file:1]
- **pyannote.audio** because it helps with speaker diarization in local or controlled AI pipelines. [file:1]
...
Typical examples:
- **Supabase** because it offers a fast path for managed PostgreSQL, authentication, and storage in early phases. [file:1]
- **Railway** because it is easy to use for early service deployment and internal environments before larger-scale infrastructure is required. [file:1]
- **Vercel** because it is convenient for frontend deployment and preview environments. [file:1]
- **Sentry** because it provides quick error monitoring with low initial setup. [file:1]
- **Better Stack** because it can provide early uptime monitoring and logging workflows before full enterprise observability maturity. [file:1]
- **Cloudflare** because even basic plans can provide useful CDN, TLS, and edge protections. [file:1]
...
Typical examples:
- **OpenAI API** because the architecture defines it as the primary LLM provider for major AI features such as summaries, question answering, extraction, and agentic workflows. [file:1]
- **AssemblyAI fallback usage** because it is part of the fallback speech pipeline when Whisper is unavailable or insufficient. [file:1]
...
Typical examples:
- **AWS ECS / Fargate** because the architecture plans a move there in later phases when load and tenant counts justify more scalable infrastructure. [file:1]
- **Upstash HA / advanced Redis setup** because higher reliability may be needed for queues and caching. [file:1]
- **Cloudflare Pro or higher** because stronger WAF, edge controls, and production-grade protections may become necessary. [file:1]
- **Doppler Team or advanced secrets management** because environment and team complexity increases over time. [file:1]
- **PagerDuty or stronger incident tooling** because production support workflows mature with scale. [file:1]
- **Managed ClickHouse or larger analytics infrastructure** because dashboard and reporting workloads grow. [file:1]
- **Alternative vector stores such as Qdrant, Weaviate, or Pinecone** if pgvector limits are reached for larger enterprise workloads. [file:1]
...
- **Python** because it is the main language for AI and ML service development in the architecture. [file:1]
...
- **OpenAI API** because it is the primary cloud LLM provider defined in the architecture. [file:1]
- **LiteLLM** because it gives a unified interface for calling multiple providers and managing fallback behavior in code. [file:1]
- **LangGraph** because it supports structured multi-step AI workflows and agents. [file:1]
- **Whisper** because it provides transcription capability.
- **AssemblyAI** because it serves as a fallback transcription route when needed. [file:1]
- **spaCy** because it helps with NLP preprocessing and extraction workflows. [file:1]
- **sentence-transformers** and embedding tools because semantic retrieval and RAG need embeddings.
- **pgvector** because it lets us store embeddings close to source data in PostgreSQL. [file:1]
...
- **scikit-learn** and **XGBoost** because forecasting and predictive tasks are better handled by classical ML in some cases than by LLMs. [file:1]
...
- **Docker** because all major services are containerized. [file:1]
- **Docker Compose** because it simplifies local orchestration. [file:1]
- **Railway** because it supports fast early deployment in Phase 1 and Phase 2. [file:1]
- **Vercel** because it supports frontend deployment and preview workflows. [file:1]
- **GitHub Actions** because it automates CI/CD checks such as linting, tests, validation, and deploys. [file:1]
- **Doppler** because secrets management should be centralized and not hardcoded into repos or images. [file:1]
- **AWS ECS / Fargate** because later scale may require more robust hosting and control. [file:1]
- **Cloudflare** because it can provide edge, CDN, TLS, and protection capabilities. [file:1]

### 5.5 Data and Storage Tools

Data and storage tools handle the system of record, caching, indexing, analytics, embeddings, files, and exports. Because the product is built around capturing and structuring revenue signals, this category is central to the architecture. [file:1]
...
- **PostgreSQL / Supabase PostgreSQL** because it is the primary relational store for platform data. [file:1]
- **pgvector** because semantic search and RAG need vector storage near core data. [file:1]
- **Redis** because it supports caching, rate limits, and queue backing. [file:1]
- **BullMQ** because background jobs need durable queue support. [file:1]
- **Meilisearch** because full-text search is important for conversation library and filtered discovery. [file:1]
- **ClickHouse** because analytical workloads benefit from a columnar engine. [file:1]
...
- **Warehouse connectors** because customers may need exports to systems like Snowflake or BigQuery. [file:1]
...
- **Sentry** because it gives visibility into production errors and performance problems. [file:1]
- **Better Stack** because it supports log aggregation, uptime monitoring, alerting, and operational visibility. [file:1]
- **Grafana** because metrics dashboards help us understand queue depth, latency, and service health. [file:1]
- **Cloudflare** because it provides TLS termination, CDN features, DDoS protection, and edge controls. [file:1]
- **Doppler** because secrets must be managed centrally and securely. [file:1]
- **PagerDuty** or equivalent later because incident response maturity increases as production scale grows. [file:1]

### 5.7 External APIs and Integrations

External APIs and integrations connect the platform to systems we do not own but need for core workflows. This is a major category because the product depends heavily on CRM data, meeting systems, email systems, and external AI services. [file:1]
...
- **Salesforce**, **HubSpot**, and **Dynamics 365** because CRM sync is core to revenue workflows. [file:1]
- **Zoom**, **Microsoft Teams**, and **Google Meet** because the platform captures meeting and call data from these ecosystems. [file:1]
- **Gmail API** and **Outlook / Microsoft Graph** because email workflows are part of capture and engagement features. [file:1]
- **OpenAI API** because it powers core AI tasks in the initial architecture. [file:1]
- **AssemblyAI** because it supports fallback speech workflows. [file:1]
- **Snowflake** and **BigQuery** connectors because customers may need downstream reporting or warehouse exports. [file:1]
...
No new tool should be added casually to the codebase, infrastructure, or workflow. The architecture document explicitly states that every technology must have an approved architecture decision before it enters the codebase, and that new alternatives should not be introduced without review. [file:1]
...
For local development, prefer open-source tools whenever practical. The architecture already leans heavily on open-source foundations such as Next.js, NestJS, FastAPI, PostgreSQL, Redis, BullMQ, Prisma, Zod, Whisper, LangGraph, and other code-level dependencies, which makes this a natural principle for the team. [file:1]
...
- use **Ollama** for local open-model testing instead of always calling paid LLM APIs [file:1][web:26]
...
- use **Whisper** for self-hosted transcription experiments and fallback speech workflows [file:1]
...
Examples:
- use **OpenAI API** for production-grade summarization and structured outputs where model quality matters most [file:1]
- upgrade **Sentry**, **Better Stack**, or **Cloudflare** when production reliability and visibility requirements increase [file:1]
- move from **Railway** to **AWS ECS / Fargate** when scale or operational requirements justify it [file:1]
- upgrade vector or analytics infrastructure when data size and performance needs exceed the MVP architecture limits [file:1]
...
- **Why it matters for this project:** Our architecture already plans a self-hosted fallback model path and uses a provider-routing layer, so Ollama is a very practical fit for evaluation and resilience planning. [file:1]
...
- **Why it matters for this project:** Our product includes retrieval-style AI features, so fast offline RAG experiments help de-risk future design work. [file:1][page:1]

#### pgvector in local PostgreSQL
- **Type:** Local vector storage
- **Offline:** Yes
- **Open Source:** Yes
- **Why we use it:** The architecture already uses pgvector as the planned vector store, so using it locally is the best way to test embeddings, semantic retrieval, and RAG behavior without introducing extra complexity. [file:1]
- **Use cases in development:**
...
- **Why it matters for this project:** Keeps local experiments aligned with the actual planned architecture. [file:1]
...
Local speech and audio tools are very important for this platform because transcription and conversation analysis are a major part of the product value. Self-hosted speech tools help reduce dependency on external APIs and support privacy-sensitive testing. [file:1]

#### Whisper
- **Type:** Local speech-to-text model
- **Offline:** Yes
- **Open Source:** Yes
- **Why we use it:** The architecture defines Whisper as the primary speech-to-text path, and explicitly notes the value of self-hosting for cost control, privacy, and independence from third-party API queue times. [file:1]
- **Use cases in development:**
...
- **Why it matters for this project:** Transcription is one of the most critical parts of the product, so a strong self-hosted option is strategically important. [file:1]

#### pyannote.audio
- **Type:** Local speaker diarization tool
- **Offline:** Yes
- **Open Source:** Yes
- **Why we use it:** The architecture uses pyannote for speaker diarization, which is necessary to identify who spoke when in conversation analysis pipelines. [file:1]
- **Use cases in development:**
  - speaker segmentation
  - conversation structure analysis
  - improved transcript labeling
  - testing diarization quality before production rollout
- **Why it matters for this project:** Accurate speaker separation improves transcript usefulness and downstream AI analysis. [file:1]
...
- **Speech pipeline testing:** Use Whisper and diarization locally to validate end-to-end ingestion and transcription flows. [file:1]
...
- architecture decisions need stable, reviewed dependencies rather than accidental provider sprawl [file:1]
...
- **Why we use it:** The architecture defines Next.js 15 as the frontend framework for the product, with App Router support, server-side rendering for initial loads, and client-side navigation for the application experience. [file:1]
...
- **Why it matters:** It gives us a strong production-ready React foundation without having to assemble routing, rendering, and app structure from scratch. [file:1]

#### TypeScript
- **Type:** Frontend language / typing layer
- **Why we use it:** The architecture defines TypeScript across the frontend to ensure type safety across props, API responses, and state shapes. This is especially important in a product that has many modules, AI outputs, and integration-driven UI states. [file:1]
...
- **Why it matters:** It reduces runtime mistakes and makes code easier to understand and maintain, especially for mixed-experience teams. [file:1]

### 10.2 UI Libraries

#### Tailwind CSS
- **Type:** Utility-first styling framework
- **Why we use it:** The architecture explicitly states that TailwindCSS is used for all styling via utility classes and that custom CSS files should not be the normal path. [file:1]
...
- **Why it matters:** It speeds up UI work, reduces CSS sprawl, and makes it easier for juniors to follow a predictable styling system. [file:1]

#### shadcn/ui
- **Type:** Component system built on Radix UI + Tailwind
- **Why we use it:** The architecture defines shadcn/ui as the chosen accessible component layer for the frontend instead of building a custom component library from scratch. [file:1]
...
- **Key use cases in development:**
  ...
  - dropdowns
  - command palettes
- **Why it matters:** It gives us modern, accessible components with low setup overhead and fits nicely with Tailwind-based styling. [file:1]

#### Radix UI
- **Type:** Accessibility-focused primitive UI library
- **Why we use it:** The architecture specifically mentions shadcn/ui as Radix-based, which means Radix primitives are part of the practical UI foundation for accessibility and composable interactions. [file:1]
...
- **Why it matters:** It lets us build polished enterprise UI without re-implementing accessibility basics ourselves. [file:1]

### 10.3 State Management

The architecture gives very clear state-management rules: server state must use TanStack Query, client-only UI state must use Zustand, form state must use React Hook Form, and URL state should use Next.js router and search params. This separation is important because it prevents the common mistake of mixing all state types into one store. [file:1]

#### TanStack Query
- **Type:** Server-state management library
- **Why we use it:** The architecture explicitly defines TanStack Query as the place for API data, caching, background refetching, and optimistic updates, and it clearly says it should not be used for UI-only state. [file:1]
...
- **Why it matters:** It gives predictable server-state caching and reduces manual loading, refetch, and stale-state bugs. [file:1]

#### Zustand
- **Type:** Client-state management library
- **Why we use it:** The architecture explicitly defines Zustand as the store for client-side UI state such as panels, filters, sidebars, and selected rows. [file:1]
...
- **Why it matters:** It is lightweight, easy to understand, and prevents misuse of server-state tools for UI-only behavior. [file:1]

#### Next.js router state
- **Type:** URL state management mechanism
- **Why we use it:** The architecture directly states that URL state such as filters and pagination should be handled using Next.js router and search params. [file:1]
...
- **Key use cases in development:**
  ...
  - tab routing
- **Why it matters:** URL state improves usability, refresh behavior, and collaboration because users can share the exact current view. [file:1]

### 10.4 Forms and Validation

#### React Hook Form
- **Type:** Form state management library
- **Why we use it:** The architecture explicitly states that form state should be handled with React Hook Form instead of general-purpose state libraries. [file:1]
...
- **Why it matters:** It keeps forms performant, structured, and easier to validate. [file:1]

#### Zod
- **Type:** Validation library
- **Why we use it:** Zod is defined in the architecture as the runtime validation layer for API request bodies and event payloads in the backend, and it also fits naturally with typed frontend form validation for consistent schema-driven development. [file:1]
...
- **Why it matters:** It reduces invalid input bugs and creates clearer contracts between UI and services. [file:1]

### 10.5 Charts and Visualization

The architecture mentions dashboards, forecast views, performance insights, and analytical outputs, which means charting and visualization are important frontend concerns even if the specific chart library is not hard-fixed in the core architecture document. The frontend should therefore use a simple, approved charting approach that integrates well with React and TypeScript while staying easy for juniors to maintain. [file:1]
...
- If we adopt another chart library later, it should still follow the core principle of keeping frontend logic simple and presentation-focused, while all calculations and business rules stay in backend or analytics layers. [file:1]
- New charting libraries should not be added without review, because visual libraries can become heavy and inconsistent across the product. [file:1]

### 10.6 Frontend Testing

Frontend testing is important because this product contains many interactive screens, filters, tables, forms, and real-time UI states. The architecture also emphasizes system-level and integration-level validation, so frontend tests must cover both UI behavior and connected flows. [file:1]

#### Playwright
- **Type:** End-to-end browser testing tool
- **Why we use it:** Playwright is a strong fit for realistic end-to-end browser testing across authenticated flows, dashboards, forms, and multi-step user interactions. The project architecture emphasizes end-to-end system behavior and QA readiness, which makes Playwright a natural approved testing tool. [file:1]
...
- **Why it matters:** It tests the product the way real users use it. [file:1]
...
- **Why we use it:** The architecture defines NestJS v11 as the core framework for the modular monolith, mapping backend modules to product modules and providing dependency injection, guards, and structured service organization. [file:1]
...
- **Why it matters:** It gives a clean enterprise-friendly structure, which is very useful when the system is divided into multiple platform modules. [file:1]

#### TypeScript
- **Type:** Backend language
- **Why we use it:** The architecture explicitly states that all product services must use TypeScript and that there are no exceptions for product feature code, APIs, event handling, CRM sync, or workflow logic. [file:1]
...
- **Key use cases in development:**
  ...
  - integration clients
- **Why it matters:** It improves maintainability and keeps frontend and backend aligned around a shared language for product engineering. [file:1]

#### Prisma ORM
- **Type:** ORM and migration tool
- **Why we use it:** The architecture defines Prisma ORM as the type-safe PostgreSQL access layer and migration workflow for the API layer. [file:1]
...
- **Key use cases in development:**
  ...
  - schema evolution
  - migration workflows
- **Why it matters:** It reduces raw-query errors and makes database access easier to reason about for both experienced and junior engineers. [file:1]

### 11.2 API and Validation

#### REST API pattern
- **Type:** API design approach
- **Why we use it:** The frontend is documented to connect to the backend through REST APIs, and the API layer is responsible for all reads and writes between UI and product services. [file:1]
...
- **Why it matters:** It keeps the product interface predictable and easy to test. [file:1]

#### Zod
- **Type:** Runtime validation library
- **Why we use it:** The architecture states that Zod is used for runtime validation of all API request bodies and event payloads and that no unvalidated request should reach business logic. [file:1]
...
- **Why it matters:** It protects the system from bad input and keeps contracts explicit. [file:1]
...
- **Why we use it:** The architecture defines Supabase Auth as the authentication layer for sign-up, sign-in, OAuth, session management, and JWT issuance, and it lists it among the key external dependencies and critical platform services. [file:1]
...
- **Why it matters:** It removes the need to build a custom auth server during early phases and helps the team ship faster. [file:1]

#### JWT Guards
- **Type:** Authorization enforcement mechanism
- **Why we use it:** The architecture explicitly states that JWT guards verify JWTs and enforce RBAC on every request and that no route should be left unguarded. [file:1]
...
- **Key use cases in development:**
  ...
  - secure module APIs
- **Why it matters:** It makes authorization enforcement explicit and consistent. [file:1]

#### RBAC
- **Type:** Role-based access control model
- **Why we use it:** The architecture describes RBAC as part of the platform-wide governance and ties users such as AE, SDR, Sales Manager, VP Sales, CRO, and RevOps to specific access roles and module usage patterns. [file:1]
...
- **Why it matters:** Revenue tools often expose sensitive data, so role boundaries must be clear from day one. [file:1]

### 11.4 Queue and Event Tools

The architecture makes event-driven communication a core principle. It clearly states that modules must communicate via events rather than direct synchronous internal calls, with BullMQ and Redis acting as the internal event-bus and queueing backbone. This is very important because the system depends on async transcription, AI workflows, retries, and inter-module processing. [file:1]

#### BullMQ
- **Type:** Queue and event orchestration library
- **Why we use it:** The architecture explicitly defines BullMQ as the internal event bus between modules and requires AI calls and inter-module async workflows to use queue-based patterns. It also requires events to be idempotent because retries can happen. [file:1]
...
- **Why it matters:** It helps decouple modules and makes heavy or slow operations safe to run asynchronously. [file:1]

#### Redis
- **Type:** Queue backing store and cache
- **Why we use it:** The architecture identifies Redis as the backing store for BullMQ and also highlights it as a critical path dependency because if Redis goes down, event processing and queue-based AI workflows stop. [file:1]
...
- **Why it matters:** It is a core operational dependency for the event-driven architecture. [file:1]

#### Event contracts
- **Type:** Architectural communication pattern
- **Why we use it:** The architecture defines named events such as `call.transcription.completed`, `revenue_graph.entity.linked`, `tracker.detection.created`, and `deal.stage.changed`, and it states that consumers must subscribe to published events rather than directly coupling to internal module code. [file:1]
...
- **Why it matters:** It protects modularity and makes later module extraction easier. [file:1]

### 11.5 Backend Testing

Backend testing is critical because the backend owns business logic, module boundaries, validation, integrations, security enforcement, and async workflows. The architecture also stresses end-to-end data flows, event verification, and integration correctness, which means backend testing must include more than simple unit tests. [file:1]
...
- Do not test AI model quality inside backend service tests; test contract handling and orchestration instead, because inference belongs to Python AI services. [file:1]


## 12. AI and ML Stack

This section defines the approved AI and ML stack for R-Revenue Intelligence. The architecture is very clear on one core rule: all AI and ML logic belongs in Python services, while TypeScript product services only orchestrate business workflows, queues, APIs, and integrations. The platform also defines OpenAI as the primary LLM provider through LiteLLM, Whisper as the primary ASR path, AssemblyAI as the speech fallback, and pgvector-based retrieval for RAG-style features. [file:1]
...
- it keeps the stack understandable even for freshers: TypeScript for product logic, Python for AI logic only [file:1]

### 12.1 Primary LLM Providers

#### OpenAI API
- **Type:** Managed LLM API
- **Why we use it:** The architecture explicitly approves OpenAI API as the primary LLM provider and records this as an ADR-level decision. It is used for summaries, Q&A, email draft generation, RAG outputs, and AI-assisted workflows across modules such as M-02, M-06, and M-10. [file:1]
...
- **Why it matters:** It is the main cloud AI capability in the approved architecture, so many downstream features depend on it. [file:1]

#### OpenAI for structured JSON output
- **Type:** Applied LLM usage pattern
- **Why we use it:** The architecture repeatedly emphasizes that AI services should return structured JSON to TypeScript services rather than free-form product logic. This makes OpenAI useful not just for text generation but for machine-readable outputs such as next steps, risks, extracted entities, and confidence scores. [file:1]
...
- **Why it matters:** Structured outputs are easier to validate, test, and safely integrate than raw text alone. [file:1]

### 12.2 Free / Backup LLM Providers

The approved architecture already anticipates provider fallback and explicitly mentions LiteLLM routing with Anthropic as a fallback path, while earlier sections of this document also include local and free-access options for experimentation and backup. That means the team should think of backup providers in two layers: approved fallback for production resilience, and free/local providers for development and experimentation. [file:1]

#### LiteLLM fallback routing
- **Type:** Provider routing strategy
- **Why we use it:** The architecture states that LiteLLM provides unified LLM gateway behavior with OpenAI as primary and Anthropic as fallback, and it also calls out the need for fallback handling in external AI dependency planning. [file:1]
...
- **Why it matters:** It reduces vendor lock-in and helps the system fail more gracefully. [file:1]

#### Anthropic fallback path
- **Type:** Secondary managed LLM provider
- **Why we use it:** The architecture mentions LiteLLM to Anthropic as the fallback path for OpenAI-backed LLM workflows. [file:1]
...
- **Why it matters:** It helps ensure that core AI features do not depend on one provider only. [file:1]
...
- **Why we use it:** The architecture explicitly defines Python as the only allowed language for AI and ML services, with TypeScript reserved for product services. [file:1]
...
- **Why it matters:** It creates a clean and non-negotiable boundary between product logic and AI logic. [file:1]

#### FastAPI
- **Type:** AI service framework
- **Why we use it:** The architecture defines FastAPI as the internal HTTP API layer for AI services and for the separate transcription service. It is used to expose endpoints such as `/v1/summarize`, `/v1/answer-query`, `/v1/embed`, `/v1/score-call`, and transcription-related endpoints. [file:1]
...
- **Why it matters:** It keeps AI services clean, fast to build, and easy to document. [file:1]

#### LiteLLM
- **Type:** LLM gateway and routing framework
- **Why we use it:** The architecture explicitly names LiteLLM as the unified LLM gateway, with OpenAI primary and Anthropic fallback. It also notes that API layer code must not call provider SDKs directly. [file:1]
...
- **Why it matters:** It prevents provider-specific code from spreading across the AI stack. [file:1]

#### LangGraph
- **Type:** Agent and workflow orchestration framework
- **Why we use it:** The architecture defines LangGraph as the stateful multi-step workflow engine for features like AI Deep Researcher and Ask Anything. [file:1]
...
- **Why it matters:** It is a good fit for complex AI tasks that need more than one prompt or one model call. [file:1]

#### BullMQ-driven async AI execution
- **Type:** Queue orchestration pattern
- **Why we use it:** The architecture explicitly says AI calls must be async through BullMQ rather than direct synchronous request-handler calls, especially for heavy or long-running tasks. [file:1]
...
- **Why it matters:** It improves reliability and keeps user-facing APIs responsive. [file:1]

### 12.4 NLP Libraries

#### spaCy
- **Type:** NLP preprocessing library
- **Why we use it:** The architecture explicitly lists spaCy as the NLP preprocessing tool for tokenization and entity extraction and also notes that it does not perform LLM calls. [file:1]
...
- **Why it matters:** Not every text problem needs an LLM, and spaCy gives us fast, structured NLP building blocks. [file:1]

#### Topic modeling and clustering utilities
- **Type:** NLP analysis tooling category
- **Why we use them:** The architecture ties conversation intelligence features to theme detection, topic tagging, and topic modeling across call data. [file:1]
...
- **Why it matters:** These are core product features in the “Understand” stage of the platform. [file:1]

#### Intent detection pipelines
- **Type:** NLP + semantic classification workflow
- **Why we use them:** The architecture explicitly describes AI Smart Tracker as intent-based signal detection using semantic NLP rather than simple keyword matching. [file:1]
...
- **Why it matters:** It turns raw conversation text into actionable structured signals. [file:1]

### 12.5 Speech-to-Text and Audio

#### Whisper
- **Type:** ASR model / transcription engine
- **Why we use it:** Whisper is explicitly approved as the primary speech-to-text path and is identified as part of the critical revenue path in the architecture. It is used in M-01 and sits at the start of much of the downstream AI pipeline. [file:1]
...
- **Why it matters:** No transcript means no downstream AI value for many modules. [file:1]

#### AssemblyAI
- **Type:** Speech API / fallback ASR provider
- **Why we use it:** The architecture explicitly approves AssemblyAI as the fallback ASR path and also ties it to speaker diarization support. [file:1]
...
- **Key use cases in development:**
  ...
  - resilience planning
- **Why it matters:** It reduces risk if Whisper-based processing is unavailable or insufficient for a specific case. [file:1]

#### pyannote.audio
- **Type:** Speaker diarization library
- **Why we use it:** The architecture explicitly lists pyannote in the transcription service as the tool for speaker diarization. [file:1]
- **Primary use:** Identify who spoke when.
- **Key use cases in development:**
  - speaker segmentation
  - cleaner transcript structure
  - improved downstream summaries and scoring
- **Why it matters:** Better speaker labeling makes call intelligence more trustworthy. [file:1]
...
- **Why we use it:** The architecture explicitly uses pgvector for semantic search and RAG, including the Ask Anything retrieval flow. [file:1]
...
- **Why it matters:** It keeps the early vector workflow aligned with the main database and avoids premature infrastructure sprawl. [file:1]

#### `/v1/embed` AI endpoint
- **Type:** Internal embedding API pattern
- **Why we use it:** The architecture explicitly defines an internal embed endpoint used by modules such as M-05 and M-06. [file:1]
...
- **Why it matters:** It gives a central place to manage embedding logic instead of spreading it across modules. [file:1]

#### sentence-transformers / local embedding models
- **Type:** Embedding model tooling
- **Why we use them:** The architecture mentions embedding generation as part of the Python AI services layer, and local embedding models are useful for testing retrieval workflows without always paying for hosted embeddings. [file:1]
...
The platform’s forecasting module is not just an LLM feature. The architecture explicitly describes revenue projection as combining execution data with historical conversion patterns and weighted pipeline modeling, which means classical ML and analytical methods are important here. [file:1]
...
- **Why we use it:** The architecture directly describes forecasting as using historical conversion rates and weighted pipeline modeling, which means some predictions will rely on explicit business logic and statistical methods, not only ML models. [file:1]
...
- **Why it matters:** Forecasting needs both accuracy and explainability for business users. [file:1]

### 12.8 AI Testing and Evaluation Tools

AI testing matters because the architecture requires structured outputs, confidence scores, fallback handling, and safe routing of flagged outputs. The system also states that low-confidence outputs should be marked for review and should not be auto-written to CRM. [file:1]
...
- **Why we use it:** The architecture explicitly states that AI endpoints return `confidence_score` and that results below a threshold should be flagged for review. [file:1]
...
- **Why it matters:** It gives an operational safety layer around AI outputs. [file:1]
...
- **Why it matters:** The application depends on predictable AI outputs, not just good-looking text. [file:1]

#### Human review workflows
- **Type:** Operational evaluation mechanism
- **Why we use it:** The architecture clearly states that flagged outputs should go to review instead of automatic CRM write-back. [file:1]
...
- **Why it matters:** It improves trust in the system and supports safer rollout. [file:1]
...
- it makes future scaling decisions easier to reason about [file:1]

### 13.1 Primary Database

#### PostgreSQL
- **Type:** Primary relational database
- **Why we use it:** PostgreSQL is explicitly approved in the architecture as the primary relational database and is captured in ADR-004. It stores users, accounts, deals, transcripts, detections, CRM sync state, and other core platform data. [file:1]
...
- **Key use cases in development:**
  ...
  - feature data models
- **Why it matters:** It is the core data backbone for the entire platform. [file:1]

#### Shared PostgreSQL with Row-Level Security
- **Type:** Multi-tenant database architecture
- **Why we use it:** The architecture explicitly defines shared PostgreSQL with RLS and `tenant_id`-based isolation as the approved multi-tenancy model and records this as ADR-008. [file:1]
...
- **Why it matters:** Multi-tenancy safety is non-negotiable in this platform. [file:1]

#### Supabase PostgreSQL
- **Type:** Managed PostgreSQL deployment option
- **Why we use it:** The architecture refers to Supabase PostgreSQL with RLS and PITR as the Phase 1 primary database setup and also uses Supabase as part of the broader managed early-stage infrastructure. [file:1]
...
- **Why it matters:** It helps the team ship quickly before moving to more customized infra at larger scale. [file:1]

### 13.2 Cache and Queue Storage

#### Redis
- **Type:** In-memory cache and queue backing store
- **Why we use it:** The architecture explicitly defines Redis as the cache layer and as the critical dependency backing BullMQ. It also calls Redis a single point of failure in early phases and highlights it as the critical path for event processing. [file:1]
...
- **Why it matters:** If Redis fails, event-driven processing stalls. [file:1]

#### BullMQ on Redis
- **Type:** Queue and event-bus storage pattern
- **Why we use it:** The architecture explicitly approves BullMQ on Redis as the event bus in ADR-005 and uses it for transcription jobs, AI processing, inter-module events, retries, and dead-letter behavior. [file:1]
...
- **Why it matters:** It is essential for the modular event-driven design. [file:1]

#### Upstash Redis HA
- **Type:** Scaled managed Redis option
- **Why we use it:** The architecture identifies Upstash Redis HA as a planned resilience upgrade path for Redis in later phases. [file:1]
...
- **Why it matters:** Important for scale and reliability planning. [file:1]

### 13.3 Search Engine

#### Meilisearch
- **Type:** Search index
- **Why we use it:** The architecture explicitly lists Meilisearch as the search index for transcripts, emails, and account/deal metadata, with typo-tolerant and fast search behavior. [file:1]
...
- **Key use cases in development:**
  ...
  - filtered search UX
- **Why it matters:** Search is a core product capability, especially for conversation archives and signal discovery. [file:1]

### 13.4 Analytics Store

#### ClickHouse
- **Type:** Columnar analytics database
- **Why we use it:** The architecture explicitly defines ClickHouse as the analytics layer for revenue metrics, coaching metrics, and activity streams, and places it in Phase 2 for analytical workloads. [file:1]
...
- **Why it matters:** Analytical workloads should not overload the transactional database. [file:1]

#### Analytics separation principle
- **Type:** Architecture rule
- **Why we use it:** The architecture separates transactional storage from search and analytics stores, which is important to keep the system scalable and predictable. [file:1]
...
- **Why it matters:** Prevents one store from becoming a bottleneck for every workload. [file:1]
...
- **Why we use it:** The architecture explicitly states that the platform owns transcripts and metadata, not long-term raw audio, and that audio should be auto-deleted after a configurable retention period, with 7 days used as the default compliance and cost-control rule. [file:1]
...
- **Why it matters:** Raw audio is expensive and sensitive, so we should not keep it longer than needed. [file:1]

### 13.6 Data Export / Warehouse Connectors

#### Data Cloud export layer
- **Type:** Data export architecture
- **Why we use it:** The architecture explicitly defines Data Cloud in M-03 as the customer data export mechanism and records a daily idempotent sync approach in ADR-009. [file:1]
...
- **Why it matters:** The architecture treats customer ownership of data as a core principle. [file:1]

#### Snowflake connector
- **Type:** Warehouse export target
- **Why we use it:** Snowflake is explicitly listed as a supported Data Cloud destination in the architecture. [file:1]
- **Primary use:** Export data to customer-owned Snowflake environments.
- **Key use cases in development:**
  - customer analytics pipelines
  - downstream reporting
  - enterprise data integration
- **Why it matters:** Important for enterprise reporting and data ownership. [file:1]

#### BigQuery connector
- **Type:** Warehouse export target
- **Why we use it:** BigQuery is explicitly listed as a supported Data Cloud destination in the architecture. [file:1]
- **Primary use:** Export data to customer-owned BigQuery environments.
- **Key use cases in development:**
  - analytics exports
  - customer-owned reporting pipelines
  - batch data sync
- **Why it matters:** Supports customer analytics ecosystems. [file:1]

#### Databricks connector
- **Type:** Warehouse / lakehouse export target
- **Why we use it:** Databricks is explicitly listed as a Data Cloud target in the architecture. [file:1]
...
- **Why it matters:** Useful for larger customers with advanced data platforms. [file:1]

#### Amazon S3 and Redshift connectors
- **Type:** Data export targets
- **Why we use them:** The architecture explicitly includes Amazon S3 and Amazon Redshift as outbound data destinations in the Data Cloud design. [file:1]
...
- **Why it matters:** Supports flexible customer-owned data strategies. [file:1]

#### Connector rule
- **Type:** Governance principle
- **Why we use it:** The architecture explicitly states that warehouse credentials are client-provided and that Relanto.ai does not own customer warehouse infrastructure or directly host customer analytics. [file:1]
...
- **Why it matters:** It keeps data ownership clear and reduces compliance risk. [file:1]
...
- it supports the approved modular monolith now and future service extraction later [file:1]

### 14.1 Local Development

Local development must stay as close as possible to the real architecture while remaining simple enough for daily engineering work. The architecture explicitly states that all services run in containers and that local development uses Docker Compose. [file:1]

#### Docker Compose
- **Type:** Local orchestration tool
- **Why we use it:** The architecture explicitly states that local development uses Docker Compose so engineers can run the main services together in a predictable setup. [file:1]
...
- **Why it matters:** It gives one simple command path for local startup and reduces environment mismatch. [file:1]

#### Local parity principle
- **Type:** DevOps operating principle
- **Why we use it:** The architecture separates product services, AI services, queueing, and storage, so local development must reflect those boundaries instead of hiding them behind fake shortcuts. [file:1]
...
- **Why it matters:** It helps catch integration problems early instead of discovering them only after deployment. [file:1]

### 14.2 Containerization

#### Docker
- **Type:** Containerization platform
- **Why we use it:** The architecture explicitly says all services run as containers and lists Docker as the standard packaging mechanism for the frontend, NestJS monolith, transcription service, and AI services layer. [file:1]
...
- **Why it matters:** It creates predictable runtime behavior and makes deployment much easier. [file:1]

#### Service-specific container boundaries
- **Type:** Architecture-aligned container strategy
- **Why we use it:** The architecture clearly separates the NestJS monolith, the transcription service, and the AI services layer because they have different runtime and scaling profiles. [file:1]
...
- **Why it matters:** Different workloads need different memory, scaling, and failure boundaries. [file:1]

### 14.3 Hosting

#### Railway
- **Type:** Managed hosting platform
- **Why we use it:** The architecture explicitly defines Railway as the deployment platform for Phase 1 and Phase 2 because it offers fast setup, health checks, restarts, and simple managed container hosting. [file:1]
...
- **Why it matters:** The architecture chose Railway because Phase 1 delivery speed is critical and the team does not need full microservice-grade infra complexity yet. [file:1]

#### AWS ECS / Fargate
- **Type:** Planned scale-up hosting platform
- **Why we use it:** The architecture explicitly states that Railway is the early hosting path and AWS ECS/Fargate is the Phase 3 upgrade target once scale triggers are met. [file:1]
...
- **Why it matters:** It gives a clear migration path without overcomplicating the MVP. [file:1]

#### Hosting migration trigger
- **Type:** Infrastructure decision rule
- **Why we use it:** The architecture explicitly defines a migration trigger from Railway to AWS ECS when tenant or request scale exceeds the current model. [file:1]
...
- **Why it matters:** Teams often waste time on enterprise infra too early; this rule helps avoid that. [file:1]

### 14.4 CI/CD

#### GitHub Actions
- **Type:** CI/CD automation platform
- **Why we use it:** The architecture explicitly defines GitHub Actions as the CI/CD layer for type checks, tests, Zod validation, Prisma validation, RLS checks, module-boundary checks, and deployment-related quality gates. [file:1]
...
- **Why it matters:** It keeps quality gates consistent and prevents broken code from reaching shared branches. [file:1]

#### PR quality gates
- **Type:** CI policy
- **Why we use it:** The architecture explicitly lists required PR checks such as type checking, tests, Zod schema validation, Prisma validation, RLS enforcement, no cross-module imports, and Sentry sourcemap upload. [file:1]
...
- **Why it matters:** Good CI acts like a second reviewer and protects the codebase from accidental regressions. [file:1]
...
- **Why we use it:** The architecture explicitly states that Doppler manages all environment variables and that no secrets should exist in repos or container images. It also lists API key rotation through Doppler for AI service integrations. [file:1]
...
- **Why it matters:** Secret sprawl is a major operational and security risk, and a central system reduces that risk. [file:1]

#### Secrets discipline
- **Type:** Security rule
- **Why we use it:** The architecture directly says secrets must not live in repos or images and must be handled centrally. [file:1]
- **Primary use:** Reduce leakage risk.
- **Key use cases in development:**
  - safe local environment setup
  - controlled rotation
  - fewer accidental credential leaks
- **Why it matters:** This is a basic but critical DevOps hygiene rule. [file:1]

### 14.6 CDN / WAF / Edge Services

#### Cloudflare
- **Type:** CDN, WAF, edge, TLS, and rate-limiting platform
- **Why we use it:** The architecture explicitly lists Cloudflare for CDN, DDoS protection, TLS termination, WAF, and rate limiting. It also specifically calls out Cloudflare rate limits as part of webhook protection for conferencing integrations. [file:1]
...
- **Why it matters:** The platform has externally exposed integration endpoints, especially webhook entry points, so edge protection is not optional. [file:1]

#### Webhook protection use case
- **Type:** Security-specific use of edge services
- **Why we use it:** The architecture explicitly says conferencing webhooks are mission-critical entry points and pairs Cloudflare rate limiting with HMAC verification and queue prioritization to prevent abuse. [file:1]
...
- the team can scale operations without losing control [file:1]

### 15.1 Error Tracking

#### Sentry
- **Type:** Error tracking platform
- **Why we use it:** The architecture explicitly states that Sentry is used for error tracking across Next.js, NestJS, and Python services, and that every exception should be logged there. [file:1]
...
- **Why it matters:** It gives one central place to find and triage runtime failures. [file:1]

#### Sentry sourcemaps and release visibility
- **Type:** Error-debugging support pattern
- **Why we use it:** The architecture includes Sentry sourcemap upload as part of CI, which improves production debugging quality. [file:1]
...
- **Why it matters:** Faster debugging means lower MTTR and fewer blind spots. [file:1]

### 15.2 Logging

#### Better Stack
- **Type:** Log aggregation and uptime platform
- **Why we use it:** The architecture explicitly lists Better Stack for log aggregation, structured log search, and uptime monitoring. [file:1]
...
- **Why it matters:** Logs are essential for debugging problems that are not obvious from error trackers alone. [file:1]

#### Structured logging discipline
- **Type:** Logging practice
- **Why we use it:** The architecture emphasizes monitored data flows, event-driven processing, and integration reliability, which all work better when logs are structured and searchable. [file:1]
...
- **Why it matters:** Unstructured logs become noise very quickly in distributed systems. [file:1]

### 15.3 Monitoring and Alerting

#### Grafana
- **Type:** Metrics and dashboard platform
- **Why we use it:** The architecture explicitly lists Grafana for dashboards covering queue depths, DB pool behavior, AI latency, and error rates, with future expansion toward distributed tracing. [file:1]
...
- **Why it matters:** Metrics help us catch degradation before users report it. [file:1]

#### Better Stack uptime monitoring
- **Type:** Uptime and alerting support
- **Why we use it:** The architecture explicitly includes Better Stack for uptime monitoring in addition to log aggregation. [file:1]
- **Primary use:** Availability checks and alerts.
- **Key use cases in development and production:**
  - service health checks
  - endpoint availability alerts
  - external-facing route monitoring
- **Why it matters:** It helps detect outage symptoms quickly. [file:1]

#### Alerting on integration and webhook failures
- **Type:** Operational safety pattern
- **Why we use it:** The architecture explicitly states that external integrations must be monitored and specifically mentions Sentry alerts and rejection-rate monitoring for webhook protections. [file:1]
...
- **Why it matters:** Many platform features depend on external systems, so silent failures are dangerous. [file:1]

### 15.4 Incident Management

#### Incident workflow maturity
- **Type:** Operational process category
- **Why we use it:** The architecture highlights failure modes, single points of failure, RTO/RPO expectations, and escalation-worthy platform risks, which means incident handling must be deliberate and not ad hoc. [file:1]
...
- **Why it matters:** A good incident process reduces panic and improves recovery speed. [file:1]

#### PagerDuty or equivalent later
- **Type:** Planned incident management tool
- **Why we use it:** While not shown as a hard Phase 1 dependency, the architecture references stronger on-call and SRE-style maturity later, making a dedicated incident platform a reasonable scale-up tool. [file:1]
- **Primary use:** On-call routing and escalation at scale.
- **Key use cases in future operations:**
  - on-call notifications
  - escalation policies
  - incident response coordination
- **Why it matters:** As the platform scales, manual alert routing becomes unreliable. [file:1]

### 15.5 Security and Compliance Tools

#### Row-Level Security (RLS)
- **Type:** Database security control
- **Why we use it:** The architecture explicitly defines shared PostgreSQL with row-level security per tenant and requires `tenant_id`-based isolation on every table and write path. [file:1]
- **Primary use:** Prevent cross-tenant data access.
- **Key use cases in development:**
  - tenant-safe queries
  - DB enforcement of isolation
  - multi-tenant protection beyond app logic
- **Why it matters:** This is one of the most important security controls in the whole platform. [file:1]

#### RBAC
- **Type:** Application authorization model
- **Why we use it:** The architecture explicitly defines role-based access control across modules and user types such as AE, SDR, Sales Manager, VP Sales, CRO, and RevOps. [file:1]
...
- **Why it matters:** Revenue intelligence data is sensitive and role boundaries must be enforced consistently. [file:1]

#### Supabase Auth + JWT
- **Type:** Authentication and token-based security stack
- **Why we use it:** The architecture explicitly defines Supabase Auth with JWT verification and guarded routes as the platform authentication pattern. [file:1]
...
- **Why it matters:** It gives us secure auth without building a custom auth system from scratch. [file:1]

#### Configure Compliance Settings
- **Type:** Platform compliance feature
- **Why we use it:** The architecture explicitly lists Configure Compliance Settings as a deployed cross-cutting feature that enforces CRM opt-out preferences and GDPR/CCPA-style policies. [file:1]
- **Primary use:** Enforce communication and data-governance rules.
- **Key use cases in development:**
  - opt-out enforcement
  - regional communication policies
  - safe automation controls
- **Why it matters:** Compliance rules must be part of platform behavior, not left to manual discipline. [file:1]

#### HMAC webhook verification
- **Type:** Integration security control
- **Why we use it:** The architecture explicitly requires HMAC-SHA256 verification for conferencing webhooks and treats webhook security as mission-critical. [file:1]
...
- **Why it matters:** Unverified webhook traffic can create cost spikes, junk ingestion, and service abuse. [file:1]

#### Audit and retention controls
- **Type:** Governance and compliance control category
- **Why we use it:** The architecture explicitly enforces audio retention limits, review processes, and client-data ownership rules. [file:1]
- **Primary use:** Reduce privacy and compliance risk.
- **Key use cases in development:**
  - audio auto-deletion
  - safe CRM field write-back
  - controlled data export behavior
- **Why it matters:** Compliance is built from many small enforced rules, not one big checkbox. [file:1]

## 16. External APIs and Integrations

This section defines the external systems that R-Revenue Intelligence connects with. The architecture is very clear that these systems sit outside the platform boundary and that we do not own them. We read from them, write approved outputs back to them, or use them as communication channels, but we do not replace their core role. [file:1]
...
- the team must know what we own and what we do not own [file:1]

### 16.1 CRM Integrations

#### Salesforce
- **Type:** External CRM integration
- **Why we use it:** The architecture explicitly lists Salesforce as a bidirectional CRM integration where accounts, contacts, opportunities, stages, and field values flow in, while AI enrichment fields, summaries, next steps, and risk signals can flow back out. [file:1]
...
- **Why it matters:** Salesforce is a common enterprise system of record, so this integration is core to platform usefulness. [file:1]

#### HubSpot
- **Type:** External CRM integration
- **Why we use it:** The architecture explicitly lists HubSpot as a bidirectional CRM integration for contacts, companies, deals, pipeline stages, AI fields, and email activity. [file:1]
...
- **Why it matters:** It broadens platform fit beyond Salesforce-heavy customers. [file:1]

#### Microsoft Dynamics 365
- **Type:** External CRM integration
- **Why we use it:** The architecture explicitly lists Dynamics 365 as a bidirectional CRM integration for accounts, contacts, opportunities, AI fields, and activity logs. [file:1]
...
- **Why it matters:** Important for enterprise customers using Microsoft ecosystems. [file:1]

#### CRM integration rule
- **Type:** Governance principle
- **Why we use it:** The architecture explicitly states that the CRM remains the system of record and that R-Revenue Intelligence should only read it and write approved AI enrichment fields, not take ownership of CRM core state. [file:1]
...
- **Why it matters:** This prevents trust and governance problems with customer systems. [file:1]

### 16.2 Meeting / Call Integrations

#### Zoom
- **Type:** Meeting / call recording integration
- **Why we use it:** The architecture explicitly lists Zoom as an inbound integration for call recordings, metadata, and participant details through HMAC-protected webhooks. [file:1]
- **Primary use:** Capture meeting and call data.
- **Key use cases in development:**
  - receive recording events
  - ingest metadata
  - trigger transcription pipelines
- **Why it matters:** It is one of the key entry points for the platform’s capture stage. [file:1]

#### Google Meet
- **Type:** Meeting integration
- **Why we use it:** The architecture explicitly lists Google Meet as an inbound source of call recordings, metadata, and participants in Phase 1. [file:1]
...
- **Why it matters:** Important for customers using Google Workspace. [file:1]

#### Microsoft Teams
- **Type:** Meeting integration
- **Why we use it:** The architecture explicitly lists Microsoft Teams as an inbound conferencing integration with HMAC-protected webhook handling in Phase 1. [file:1]
- **Primary use:** Capture conversations from Microsoft collaboration environments.
- **Key use cases in development:**
  - recording ingestion
  - metadata intake
  - participant capture
- **Why it matters:** Important for enterprise customers in Microsoft ecosystems. [file:1]

#### Dialers / telephony tools
- **Type:** Call recording integration category
- **Why we use it:** The architecture explicitly includes telephony and dialer systems as inbound sources of call recordings, metadata, and outcomes. [file:1]
...
- **Why it matters:** The product is not a dialer, but it must still work with telephony-generated conversation data. [file:1]

### 16.3 Email Integrations

#### Gmail
- **Type:** Email and calendar integration
- **Why we use it:** The architecture explicitly lists Gmail as a bidirectional integration where email content, metadata, and calendar events flow in, while AI-composed emails flow out using OAuth2. [file:1]
...
- **Why it matters:** Email is a major signal source in revenue workflows. [file:1]

#### Outlook / Office 365
- **Type:** Email and calendar integration
- **Why we use it:** The architecture explicitly lists Outlook / Office 365 as a bidirectional email and calendar integration, also based on OAuth2 delegation. [file:1]
...
- **Why it matters:** Important for enterprise Microsoft environments. [file:1]

#### Email boundary rule
- **Type:** Platform boundary principle
- **Why we use it:** The architecture explicitly states that R-Revenue Intelligence does not own email infrastructure and must use Gmail or Outlook APIs only, with no Relanto.ai MX-based sending model. [file:1]
...
- **Why it matters:** This keeps scope and deliverability risk under control. [file:1]

### 16.4 Messaging / Collaboration Integrations

#### Slack
- **Type:** Messaging and collaboration integration
- **Why we use it:** The architecture explicitly lists Slack as an outbound integration for alerts, notifications, and to-do reminders, planned for Phase 3. [file:1]
...
- **Key use cases in development:**
  ...
  - reminder delivery
  - workflow visibility
- **Why it matters:** Collaboration tools help surface signals where teams already work. [file:1]

#### Collaboration integration rule
- **Type:** Workflow extension principle
- **Why we use it:** The architecture treats collaboration tools as outbound channels, not as core systems of record. [file:1]
- **Primary use:** Extend product usefulness beyond the app UI.
- **Key use cases in development:**
  - send notifications
  - deliver reminders
  - surface action items
- **Why it matters:** It keeps the platform workflow-friendly without expanding its scope too much. [file:1]

### 16.5 Billing Integrations

#### Billing provider placeholder
- **Type:** Business operations integration category
- **Why we use it:** The current architecture excerpt does not define a formal billing provider such as Stripe, so billing integrations should be treated as not-yet-approved and require explicit review before inclusion. [file:1]
- **Primary use:** Future subscription and payment workflows.
- **Key use cases in future development:**
  - subscription management
  - invoicing support
  - plan and seat billing
- **Why it matters:** Billing is important, but it should not be added casually outside the approved architecture. [file:1]

#### Recommendation for billing tools
- **Type:** Governance note
- **Why we use it:** Because billing is not currently part of the approved architecture stack, any future billing tool should go through the same tool review process as other external systems. [file:1]
...
- **Why it matters:** Business tooling can create deep long-term dependencies too. [file:1]

### 16.6 Other Third-Party Services

#### OpenAI API
- **Type:** External AI service
- **Why we use it:** The architecture explicitly lists OpenAI API as a core external dependency for summaries, Q&A, email drafts, and RAG workflows, called only from Python AI services. [file:1]
...
- **Why it matters:** It is one of the highest-value external services in the stack. [file:1]

#### Whisper / AssemblyAI
- **Type:** External AI / speech services
- **Why we use them:** The architecture explicitly lists Whisper as the primary ASR dependency and AssemblyAI as the fallback and diarization-support path. [file:1]
- **Primary use:** Speech-to-text and backup audio intelligence.
- **Key use cases in development:**
  - transcript generation
  - fallback transcription
  - speaker-aware transcript workflows
- **Why it matters:** Transcription is the first step for much of the product value chain. [file:1]

#### LinkedIn Sales Navigator
- **Type:** Sales intelligence integration
- **Why we use it:** The architecture explicitly lists LinkedIn Sales Navigator as an inbound source of prospect profiles and connection activity in Phase 2. [file:1]
- **Primary use:** Prospect enrichment and sales context.
- **Key use cases in development:**
  - prospect context capture
  - engagement intelligence
  - sales research workflows
- **Why it matters:** Helps connect external prospect context to revenue workflows. [file:1]

#### Warehouse targets
- **Type:** Customer-owned export destinations
- **Why we use them:** The architecture explicitly lists Snowflake, BigQuery, Databricks, Amazon S3, and Redshift as outbound data-export targets through Data Cloud. [file:1]
...
- AI evaluation tests help us improve prompts and outputs without breaking trust [file:1]
...
- **Why we use it:** The architecture explicitly includes Jest in PR quality gates and requires unit plus integration coverage before merge. [file:1]
...
- **Why it matters:** It is the standard testing layer for TypeScript-heavy product code and works well with both frontend and backend workflows. [file:1]

#### pytest
- **Type:** Python unit testing framework
- **Why we use it:** The architecture separates Python AI services from TypeScript product services, so Python-side logic needs its own reliable unit test layer. [file:1]
...
- **Why it matters:** AI services should be tested as real software, not treated like magic. [file:1]

#### Unit testing rule
- Use unit tests for isolated logic only.
- Do not use unit tests as a replacement for integration or event-flow tests.
- Mock expensive or external dependencies in unit tests, especially LLM calls, speech APIs, and external integrations. [file:1]

### 17.2 Integration Testing

Integration tests verify that real components work together correctly. In this project, integration testing is especially important because the architecture depends on service boundaries, queues, schemas, OAuth-style integrations, multi-tenant data protection, and event-driven workflows. [file:1]
...
- **Why we use it:** The architecture depends on PostgreSQL, Redis, and queue-based processing, so realistic infrastructure-backed tests are much more valuable than pure mocks. [file:1]
...
- **Why it matters:** It increases confidence that the application will behave correctly outside the developer laptop. [file:1]

#### Prisma + Zod + RLS validation scripts
- **Type:** Schema and governance integration checks
- **Why we use them:** The architecture explicitly requires Prisma schema validation, Zod schema validation, and RLS enforcement checks on every PR. [file:1]
...
- **Why it matters:** These checks protect platform correctness and customer isolation, not just developer convenience. [file:1]
...
- Use integration tests whenever the logic crosses:
  ...
  - auth boundaries
  - event boundaries
- If the feature depends on another module, test the public contract, not internal private functions. [file:1]

### 17.3 End-to-End Testing

End-to-end tests validate real product workflows across multiple layers, such as frontend, backend, auth, queues, and storage. These are especially important for workflows like login, transcription, CRM setup, dashboard loading, and key user journeys. [file:1]
...
- **Why we use it:** The architecture explicitly defines event-flow verification as a deployment rule and gives concrete examples such as `call.transcription.completed -> revenue_graph.entity.linked`. [file:1]
...
- **Why it matters:** In this platform, successful E2E behavior depends heavily on events, not just screens. [file:1]

#### E2E testing rule
- Use Playwright for UI journeys.
- Use event-flow system tests for async backend journeys.
- Never mark a module as production-ready only from unit test success; upstream and downstream flow must also work with realistic data. [file:1]

### 17.4 Load and Performance Testing

Load and performance testing is important because the architecture already identifies scale-sensitive areas such as webhook ingestion, transcription throughput, Redis queue health, AI latency, PostgreSQL load, and future scaling triggers. [file:1]
...
- **Why we use them:** The architecture explicitly identifies Redis and BullMQ as critical path dependencies and documents scaling settings for transcription and AI services. [file:1]
...
- **Why it matters:** A fast API is not enough if async jobs silently back up in the background. [file:1]
...
- Include realistic payload sizes, especially for transcripts and audio-triggered workflows. [file:1]
...
- **Why we use them:** The architecture uses structured transcripts, CRM-linked entities, and AI-generated JSON outputs, so stable example payloads are very useful. [file:1]
...
- **Why we use them:** The architecture is container-first and local development already uses Docker Compose, so realistic container-backed testing is a natural fit. [file:1]
...
- **Why we use it:** The architecture explicitly requires AI services to return `confidence_score` and route low-confidence outputs to review instead of direct automation. [file:1]
...
- **Why it matters:** It adds a practical safety net around AI features. [file:1]

#### Human review and prompt review loops
- **Type:** Human-in-the-loop evaluation process
- **Why we use it:** The architecture explicitly supports flagged review flows and avoids blind automation for uncertain AI outputs. [file:1]
...
- **Why it matters:** Human review is often the fastest path to trustworthy AI improvement. [file:1]

## 18. Tool Approval Status

This table tracks whether a tool is approved, already in use, planned for later, optional for experimentation, or explicitly rejected. The architecture makes it clear that technology choices are governed decisions, not personal preferences, and that new shared tools should not enter the codebase without review. [file:1]
...
|------|----------|--------|---------|----------|----------|----------------|---------------|-------|
| Next.js 15 | Yes | Yes | No | No | No | Tech Lead / Frontend Lead | Frontend framework | Approved in architecture [file:1] |
| Tailwind CSS | Yes | Yes | No | No | No | Frontend Lead | Frontend styling | Approved in architecture [file:1] |
| shadcn/ui | Yes | Yes | No | No | No | Frontend Lead | Reusable UI components | Approved in architecture [file:1] |
| TanStack Query | Yes | Yes | No | No | No | Frontend Lead | Server-state management | Approved in architecture [file:1] |
| Zustand | Yes | Yes | No | No | No | Frontend Lead | Client-state management | Approved in architecture [file:1] |
| React Hook Form | Yes | Yes | No | No | No | Frontend Lead | Form handling | Frontend form standard [file:1] |
| Zod | Yes | Yes | No | No | No | Backend Lead | Validation | Approved in architecture [file:1] |
| NestJS v11 | Yes | Yes | No | No | No | Tech Lead / Backend Lead | Backend framework | Approved in architecture [file:1] |
| Prisma | Yes | Yes | No | No | No | Backend Lead | ORM and migrations | Approved in architecture [file:1] |
| Supabase Auth | Yes | Yes | No | No | No | Tech Lead / Backend Lead | Authentication | Approved in architecture [file:1] |
| BullMQ | Yes | Yes | No | No | No | Backend Lead | Queue and event bus | Approved in architecture [file:1] |
| Redis | Yes | Yes | No | No | No | Backend Lead / DevOps Lead | Queue backing and cache | Critical path dependency [file:1] |
| FastAPI | Yes | Yes | No | No | No | AI Lead | Python AI services | Approved in architecture [file:1] |
| LiteLLM | Yes | Yes | No | No | No | AI Lead | LLM routing | Approved in architecture [file:1] |
| OpenAI API | Yes | Yes | No | No | No | AI Lead / Tech Lead | Primary LLM inference | Core external dependency [file:1] |
| Whisper | Yes | Yes | No | No | No | AI Lead | Primary ASR | Critical path [file:1] |
| AssemblyAI | Yes | Yes | No | No | No | AI Lead | Backup ASR / diarization | Approved fallback [file:1] |
| LangGraph | Yes | No | Yes | No | No | AI Lead | Agent and workflow orchestration | Planned AI orchestration layer [file:1] |
| spaCy | Yes | No | Yes | No | No | AI Lead | NLP preprocessing | Approved in AI stack [file:1] |
| pgvector | Yes | No | Yes | No | No | Data Lead / AI Lead | Embeddings and semantic search | Approved vector strategy [file:1] |
| Meilisearch | Yes | No | Yes | No | No | Backend Lead / Search Owner | Search indexing | Planned search engine [file:1] |
| ClickHouse | Yes | No | Yes | No | No | Data Lead | Analytics store | Planned analytics layer [file:1] |
| Docker | Yes | Yes | No | No | No | DevOps Lead | Containerization | Approved in architecture [file:1] |
| Docker Compose | Yes | Yes | No | No | No | DevOps Lead | Local development | Local standard [file:1] |
| Railway | Yes | Yes | No | No | No | DevOps Lead / Tech Lead | Phase 1–2 hosting | Planned upgrade later [file:1] |
| AWS ECS / Fargate | Yes | No | Yes | No | No | Tech Lead / DevOps Lead | Phase 3 hosting | Upgrade target [file:1] |
| GitHub Actions | Yes | Yes | No | No | No | DevOps Lead | CI/CD | Approved in architecture [file:1] |
| Doppler | Yes | Yes | No | No | No | DevOps Lead / Security Owner | Secrets management | Approved in architecture [file:1] |
| Cloudflare | Yes | Yes | No | No | No | DevOps Lead / Security Owner | CDN / WAF / rate limiting | Approved in architecture [file:1] |
| Sentry | Yes | Yes | No | No | No | DevOps Lead | Error tracking | Approved in architecture [file:1] |
| Better Stack | Yes | Yes | No | No | No | DevOps Lead | Logs and uptime | Approved in architecture [file:1] |
| Grafana | Yes | Yes | No | No | No | DevOps Lead | Metrics dashboards | Approved in architecture [file:1] |
| Playwright | Yes | Yes | No | No | No | QA Lead / Frontend Lead | E2E testing | Recommended test stack |
| Jest | Yes | Yes | No | No | No | Backend Lead / Frontend Lead | Unit testing | Required in CI [file:1] |
...
| openai npm package in TypeScript services | No | No | No | No | Yes | Tech Lead | Direct LLM calls from product services | Explicitly forbidden by architecture [file:1] |
| LangChain in TypeScript product services | No | No | No | No | Yes | Tech Lead | AI logic inside product layer | Explicitly forbidden by architecture [file:1] |
| Direct cross-module DB access | No | No | No | No | Yes | Tech Lead | Shortcut data sharing | Violates module ownership rules [file:1] |
| Raw SMTP / custom email server | No | No | No | No | Yes | Tech Lead / Security Owner | Direct email infrastructure ownership | Explicitly outside platform boundary [file:1] |

## 19. Payment Planning

This table helps the team plan which tools are already likely to cost money, which ones can stay on free tiers for now, and what should trigger upgrades later. The architecture already defines clear phase changes such as Railway to AWS ECS, Redis to HA, and scale-based upgrade triggers, so payment planning should be tied to those real technical milestones rather than vague guesses. [file:1]
...
|----------------|-------------|--------------------------|-----------------------|---------------|----------|-------|
| OpenAI API | Production usage-based | When AI features expand beyond current Phase 1 scope or token usage rises sharply | High, usage-based | Summaries, Q&A, email drafts, structured AI outputs | High | Core AI dependency; architecture says this is primary LLM path [file:1] |
| Whisper | Production usage-based or self-hosted cost | Higher audio volume, lower latency needs, or more private deployment requirements | Medium to High | Speech-to-text transcription | High | Critical revenue path [file:1] |
| AssemblyAI | Production fallback usage | Increased fallback traffic or diarization-heavy usage | Medium | Backup ASR and diarization | Medium | Fallback only, so cost depends on failure and quality needs [file:1] |
| Railway | Starter / managed hosting | More than 500 tenants or major traffic growth | Medium to High | Phase 1–2 hosting | High | Architecture defines AWS ECS as upgrade path [file:1] |
| AWS ECS / Fargate | Not yet used | When Railway scale trigger is reached | High | Phase 3 hosting and service isolation | High | Upgrade only when justified by scale [file:1] |
| Supabase PostgreSQL | Managed early-stage plan | Higher DB load, enterprise requirements, or infra control needs | Medium to High | Primary relational database | High | Early-phase managed DB path [file:1] |
| Redis / Upstash HA | Starter / single-node now | Any Redis downtime incident or stronger HA need | Medium | Queue backing and cache | High | Architecture explicitly says upgrade immediately on downtime incident [file:1] |
| Cloudflare | Basic / starter | Increased traffic, stronger WAF needs, or advanced rate-limiting requirements | Low to Medium | CDN, WAF, TLS, rate limiting | High | Important even when cost is modest [file:1] |
| Sentry | Free tier or starter | Higher event volume, release complexity, or team debugging needs | Medium | Error tracking | Medium | Useful to upgrade once real production noise grows [file:1] |
| Better Stack | Free tier or starter | Higher log volume, longer retention needs, or more uptime checks | Medium | Logs and uptime monitoring | Medium | Upgrade with production maturity [file:1] |
| Grafana stack | Starter / low-cost setup | More metrics, tracing, and multi-service visibility needs | Medium | Metrics dashboards | Medium | More valuable as services and traffic grow [file:1] |
| Doppler | Starter / team plan | More environments, stronger secret governance, or larger engineering team | Low to Medium | Secrets management | Medium | Small cost, high value for hygiene [file:1] |
| GitHub Actions | Included / usage-based | Higher CI minutes, more test matrix builds, or more preview pipelines | Low to Medium | CI/CD automation | Medium | Usually grows naturally with team and test coverage [file:1] |
| ClickHouse | Not yet used | Analytics volume grows and dashboard workloads need separate scaling | Medium to High | Analytical storage | Medium | Planned Phase 2 analytics layer [file:1] |
| Meilisearch | Not yet used | Search experience becomes central and transcript corpus grows | Medium | Product search | Medium | Planned search engine [file:1] |
| pgvector | Included in PostgreSQL now | Any tenant approaches large vector limits or search performance degrades | Low to Medium initially | Embeddings and semantic search | Medium | Architecture warns about larger-scale vector triggers [file:1] |
| Dedicated vector store later | Not yet used | Any tenant exceeds around 800k vectors or retrieval performance becomes a problem | Medium to High | Large-scale vector retrieval | Low initially | Future upgrade path implied by architecture trigger notes [file:1] |
...
| Slack integration | Not yet used | When Phase 3 notifications and reminders are enabled | Low to Medium | Alerts and reminders | Low | Outbound integration planned later [file:1] |
| Snowflake / BigQuery / warehouse export ops | Not yet used | When Data Cloud is activated for customers | Medium | Customer-owned data export | Medium | Customer credentials owned by client; our cost mainly operational [file:1] |
...
- the paid plan clearly improves product quality enough to justify the spend [file:1]
...
This section captures the main technical and operational risks in the current stack. These are not theoretical risks; most of them come directly from architecture choices such as external AI dependency, modular extraction later, shared PostgreSQL with RLS, queue-based processing, and Phase 1 to Phase 3 migration plans. [file:1]

### Vendor lock-in risks
- **OpenAI dependency risk:** OpenAI is the primary LLM provider, so core AI workflows such as summaries, Q&A, and generation are sensitive to provider pricing, API changes, model deprecations, and rate limits. The architecture reduces this risk by routing through LiteLLM and keeping fallback paths available. [file:1]
- **Railway dependency risk:** Railway is the approved hosting platform for Phase 1 and Phase 2, which is good for speed but creates a platform dependency until AWS ECS migration is completed. [file:1]
- **Supabase dependency risk:** Supabase is used for PostgreSQL and authentication in the early architecture, so both data and auth have managed-service dependency risk. The document already identifies Supabase Auth and PostgreSQL as critical external dependencies. [file:1]
- **Cloud provider migration complexity:** Even when abstractions exist, moving from managed services to self-managed or AWS-native infrastructure later will still require testing, operational work, and migration planning. [file:1]

### API cost risks
- **LLM cost concentration:** The architecture explicitly indicates that OpenAI usage is a major AI budget driver, with much of product value depending on transcription and downstream AI processing. [file:1]
- **Speech processing cost risk:** Whisper and AssemblyAI usage can grow quickly when call volume increases because raw audio ingestion is a high-volume path in the system. [file:1]
- **Retry amplification risk:** Because the system is event-driven and queue-based, bad retry policies or duplicate event handling can multiply AI and transcription cost if idempotency is not enforced correctly. [file:1]
- **Silent cost growth risk:** AI systems can generate cost growth before revenue growth if prompt size, transcript size, or background AI jobs increase faster than customer conversion. [file:1]

### Free-tier limitations
- **Hosting free-tier limits:** Free or starter plans are fine for early development, but production traffic, logs, queues, and metrics can exceed free-tier limits very quickly. The architecture already assumes eventual upgrades for Redis HA, AWS hosting, and higher operational maturity. [file:1]
- **Monitoring retention limits:** Free tiers for error tracking, logs, and observability often have poor retention, lower event quotas, or limited team visibility. [file:1]
...
- **Search and analytics growth limits:** Search, embeddings, and analytics can start small on shared systems, but larger transcript and event volumes will eventually force dedicated scaling decisions such as ClickHouse maturity or vector-search optimization. [file:1]

### Privacy risks for free online tools
- **Sensitive data exposure risk:** The architecture clearly treats customer data ownership, tenant isolation, and consent as non-negotiable, so free online tools must never be used casually with customer transcripts, CRM data, or sensitive business context. [file:1]
...
- **Workspace leakage risk:** If team members paste customer data into public chat tools during testing, that becomes a governance problem even if the product architecture itself is secure. [file:1]
- **Safer rule:** Free online tools should be limited to synthetic data, sanitized samples, or public information unless explicitly approved. [file:1]

### Migration risks
- **Railway to AWS migration risk:** The architecture explicitly plans Railway in Phase 1–2 and AWS ECS in Phase 3, which means deployment, networking, scaling, secrets, observability, and incident workflows will all need migration effort later. [file:1]
- **Module extraction risk:** The architecture uses a modular monolith now and plans service extraction later, especially for M-03, M-05, and M-07. Even with clean boundaries, extraction still carries deployment, latency, and ownership risk. [file:1]
- **Search and analytics migration risk:** Moving from early simpler setups to dedicated search or analytics layers can introduce backfill, schema sync, and consistency risks. [file:1]
- **Auth and infra migration risk:** The architecture already notes fallback or future alternatives such as Auth0 and stronger Redis HA, which means identity and infra migration paths must remain documented and tested. [file:1]

### Security considerations
- **Webhook security is critical:** The architecture explicitly calls conferencing webhooks mission-critical and requires HMAC-SHA256 verification, idempotency, Cloudflare rate limiting, queue prioritization, and alerting. [file:1]
- **Tenant isolation must never rely on one layer only:** The architecture enforces defense in depth through RLS, Prisma middleware, JWT guards, tenant interceptors, and RBAC. [file:1]
- **Secrets leakage risk:** API keys and service credentials must stay in Doppler or approved secret storage and must never be hardcoded in repos, images, or local files committed to source control. [file:1]
- **TypeScript AI-call violation risk:** The architecture explicitly forbids direct AI calls from TypeScript product services, because it increases blast radius and breaks the approved separation of concerns. [file:1]
- **Retention and deletion risk:** Raw audio is sensitive and expensive, so the document explicitly requires configurable auto-deletion, with 7 days used as the default retention rule. [file:1]

### General notes
- **Current reality:** Phase 1 is strong for capture and transcription, but many downstream modules still depend on M-03 and later stages that are not fully deployed yet. [file:1]
- **Business promise risk:** Because the product is sold as modular, any architecture shortcut that weakens module independence directly threatens pricing and packaging strategy. [file:1]
- **Freshers rule:** If a shortcut creates tighter coupling, bypasses events, bypasses tenant rules, or mixes AI logic into product code, it is probably the wrong decision for this platform. [file:1]

## 21. Libraries That Will Be Used

This section lists the major libraries, frameworks, and core tools used across the platform, along with their main use cases. The selections below are based on the approved architecture, documented external dependencies, and the stack choices already established in the earlier sections. [file:1]

### Frontend libraries

| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| Next.js 15 | Frontend | Main web application framework, routing, SSR, layouts [file:1] |
| TypeScript | Frontend | Type safety for components, API usage, and state [file:1] |
| Tailwind CSS | Frontend | Utility-first styling and design consistency [file:1] |
| shadcn/ui | Frontend | Reusable accessible UI components [file:1] |
| Radix UI | Frontend | Accessible UI primitives for dialogs, popovers, menus [file:1] |
| TanStack Query | Frontend | Server-state fetching, caching, refetching, optimistic updates [file:1] |
| Zustand | Frontend | Lightweight client-side UI state [file:1] |
| React Hook Form | Frontend | Form state handling [file:1] |
| Zod | Frontend / Shared contracts | Form and payload validation, schema validation [file:1] |
...
| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| NestJS v11 | Backend | Main backend modular monolith framework [file:1] |
| TypeScript | Backend | Product services, APIs, queues, business logic [file:1] |
| Prisma ORM | Backend | PostgreSQL access, migrations, typed queries [file:1] |
| Zod | Backend | Runtime validation of requests and event payloads [file:1] |
| BullMQ | Backend | Queueing, async processing, inter-module events [file:1] |
| Redis | Backend infra | Queue backing store, caching, temporary operational state [file:1] |
| Supabase Auth SDK / JWT tools | Backend auth | Authentication and guarded access [file:1] |
...
| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| Python 3.12 | AI/ML | All AI and ML service implementation [file:1] |
| FastAPI | AI/ML | Internal AI APIs such as summarize, embed, transcribe, answer-query [file:1] |
| LiteLLM | AI/ML | LLM provider routing and fallback handling [file:1] |
| LangGraph | AI/ML | Stateful multi-step AI workflows and agent orchestration [file:1] |
| spaCy | AI/ML | NLP preprocessing, tokenization, entity extraction [file:1] |
| Whisper | AI/ML | Primary speech-to-text transcription [file:1] |
| AssemblyAI | AI/ML | Backup ASR and diarization support [file:1] |
| pyannote.audio | AI/ML | Speaker diarization [file:1] |
| sentence-transformers | AI/ML | Local embeddings and semantic similarity workflows [file:1] |
...
| OpenAI API client libraries | AI/ML | Managed LLM inference through Python services only [file:1] |

### Data and search libraries / tools

| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| PostgreSQL 16 | Data | Primary relational database [file:1] |
| pgvector | Data | Vector storage for embeddings and semantic retrieval [file:1] |
| Meilisearch | Search | Full-text and conversation search [file:1] |
| ClickHouse | Analytics | High-volume analytical queries and dashboard metrics [file:1] |
| Supabase PostgreSQL | Data platform | Managed Postgres with RLS and PITR in early phases [file:1] |

### DevOps and observability tools

| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| Docker | DevOps | Containerization for all services [file:1] |
| Docker Compose | DevOps | Local multi-service development [file:1] |
| GitHub Actions | DevOps | CI/CD pipelines and quality gates [file:1] |
| Railway | Hosting | Phase 1–2 hosting [file:1] |
| AWS ECS / Fargate | Hosting | Phase 3 scale-up hosting [file:1] |
| Doppler | DevOps | Secrets management [file:1] |
| Cloudflare | Edge / Security | CDN, WAF, TLS, DDoS protection, rate limiting [file:1] |
| Sentry | Observability | Error tracking [file:1] |
| Better Stack | Observability | Logs and uptime monitoring [file:1] |
| Grafana | Observability | Metrics dashboards and alerts [file:1] |

### Testing libraries

| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| Jest | Testing | Unit testing for TypeScript services and utilities [file:1] |
...
| Library / Tool | Layer | Use Case |
|----------------|-------|----------|
| Salesforce APIs / SDKs | Integration | CRM sync and enrichment [file:1] |
| HubSpot APIs / SDKs | Integration | CRM sync and enrichment [file:1] |
| Dynamics 365 APIs / SDKs | Integration | CRM sync and enrichment [file:1] |
| Gmail API | Integration | Email and calendar sync, delegated sending [file:1] |
| Outlook / Office 365 API | Integration | Email and calendar sync, delegated sending [file:1] |
| Zoom webhooks / APIs | Integration | Meeting ingestion and recording metadata [file:1] |
| Google Meet integrations | Integration | Meeting ingestion [file:1] |
| Microsoft Teams webhooks / APIs | Integration | Meeting ingestion [file:1] |
| Slack API / webhooks | Integration | Notifications and reminders [file:1] |
| LinkedIn Sales Navigator APIs | Integration | Prospect and sales context enrichment [file:1] |

### Important architecture note
- **No AI npm libraries in TypeScript product services.** The architecture explicitly forbids putting AI inference logic into the TypeScript product layer. AI and ML belong only in Python services. [file:1]
- **No direct cross-module DB writes.** Modules must communicate through public APIs or events, not by touching another module’s private schema. [file:1]
- **No custom SMTP or owned email server logic.** Email must go through delegated Gmail or Outlook APIs only. [file:1]

## 23. Change Log

Use this table to track updates to this document section by section. The core SAD already defines a controlled document update process, mandatory Tech Lead approval, version updates, and team notification after merge. [file:1]

| Version | Date | Updated By | Changes |
|---------|------|------------|---------|
| 1.0 | 2026-04-22 | Tech Lead / AI Assistant Draft Support | Initial draft covering tools, stack, risks, libraries, approval status, payment planning, infrastructure, observability, integrations, and testing based on current SAD [file:1] |
...
Operationally, the biggest current reality is that only early-stage modules are truly deployable today. **M-01 Data Ingestion** and cross-cutting compliance controls are live, while **M-03 Revenue Graph** is the next essential platform milestone. Many higher-value modules such as Smart Tracking, Deal Boards, Forecasting, Dashboards, and Coaching depend on M-03 and later event flows, so they must not be sold or promised until those dependencies are deployed and passing production data successfully. [file:1]

The document also establishes governance rules for tools, upgrades, and changes. Every new shared tool or platform-level change must go through Tech Lead review, the architecture document must be updated for shared decisions, and document reviews happen every three months or immediately after major architecture changes. This ensures the system stays understandable for freshers, scalable for senior engineers, and commercially aligned for the business. [file:1]

### Detailed summary table

| Area | What the document defines | Current approved choice | Current status | Main dependency / rule | Why it matters |
|------|---------------------------|-------------------------|----------------|------------------------|----------------|
| Product purpose | AI revenue intelligence platform for capture, analysis, execution, forecasting, and coaching | Revenue intelligence platform built around conversation capture and AI processing [file:1] | Active architecture baseline [file:1] | Must support 7-stage lifecycle [file:1] | Keeps all teams aligned on what the product is and is not [file:1] |
| Architecture style | System-wide structural pattern | Modular Monolith in Phase 1–2, Microservices extraction in Phase 3 [file:1] | Approved [file:1] | Extract only when all 5 triggers are met [file:1] | Avoids premature complexity while preserving future scale [file:1] |
| Commercial model | Why architecture must stay modular | Module-based licensing starting from low-cost entry pricing [file:1] | Core constraint [file:1] | No tight cross-module coupling [file:1] | Directly affects pricing and packaging viability [file:1] |
| Lifecycle model | Data and value flow across the platform | Capture → Model → Understand → Analyze → Execute → Predict → Optimize [file:1] | Approved [file:1] | Downstream modules require upstream outputs [file:1] | Prevents teams from building features without data readiness [file:1] |
| Module model | Logical packaging of platform features | 10 modules M-01 to M-10 [file:1] | Approved [file:1] | Modules own schema, APIs, and events [file:1] | Enables future extraction and clean ownership [file:1] |
| Live module path | What can actually run today | M-01 deployed; M-03 is next critical step [file:1] | Partial rollout [file:1] | M-03 required for most downstream features [file:1] | Protects delivery planning and sales promises [file:1] |
| Frontend stack | UI framework and client state approach | Next.js 15, TypeScript, Tailwind CSS, shadcn/ui, TanStack Query, Zustand, React Hook Form [file:1] | Approved and in use [file:1] | Frontend should not contain business logic [file:1] | Standardizes UI delivery and onboarding [file:1] |
| Backend stack | Product service foundation | NestJS, TypeScript, Prisma, PostgreSQL, BullMQ, Redis, Supabase Auth [file:1] | Approved and in use [file:1] | Business logic stays in TypeScript only [file:1] | Gives strong typing, modularity, and queue orchestration [file:1] |
| AI/ML stack | AI inference and processing layer | Python, FastAPI, LiteLLM, OpenAI API, Whisper, AssemblyAI, spaCy, LangGraph [file:1] | Approved [file:1] | AI must live in Python services only [file:1] | Keeps AI complexity isolated from product services [file:1] |
| Data layer | Main transactional and retrieval stores | PostgreSQL + RLS + pgvector, Meilisearch, ClickHouse [file:1] | Approved; some parts planned by phase [file:1] | Source of truth stays in PostgreSQL [file:1] | Supports transactions, search, analytics, and semantic retrieval [file:1] |
| Tenant isolation | Multi-tenant security model | Shared PostgreSQL with tenant_id and RLS [file:1] | Mandatory [file:1] | Every write must include tenant_id and every table needs RLS [file:1] | Prevents cross-tenant data leakage [file:1] |
| Event model | Inter-module communication pattern | BullMQ + Redis event-driven communication [file:1] | Approved [file:1] | No illegal direct module imports or DB shortcuts [file:1] | Preserves modular boundaries and async scalability [file:1] |
| Critical events | Minimum cross-module event chain | call.transcription.completed, revenue_graph.entity.linked, tracker.detection.created, deal.stage.changed and others [file:1] | Some live, some planned [file:1] | Event contracts owned by publisher [file:1] | Event flow is the backbone of downstream features [file:1] |
| Hosting | Deployment platform by phase | Railway now, AWS ECS/Fargate later [file:1] | Railway current, AWS planned [file:1] | Migrate only when scale triggers are hit [file:1] | Balances delivery speed with future control [file:1] |
| Local development | How engineers run the system | Docker + Docker Compose [file:1] | Approved and in use [file:1] | Local should mirror service boundaries [file:1] | Makes onboarding easier and reduces machine-specific issues [file:1] |
| CI/CD | Automated validation and delivery | GitHub Actions [file:1] | Approved and in use [file:1] | PR checks required before merge [file:1] | Keeps quality gates consistent across the team [file:1] |
| Secrets management | Secure config storage | Doppler [file:1] | Approved and in use [file:1] | No secrets in code or images [file:1] | Reduces credential leakage risk [file:1] |
| Edge and protection | Internet-facing protection layer | Cloudflare [file:1] | Approved and in use [file:1] | Used for WAF, TLS, and rate limiting [file:1] | Critical for webhook and public endpoint security [file:1] |
| Observability | Error, log, uptime, and metrics stack | Sentry, Better Stack, Grafana [file:1] | Approved and in use [file:1] | Must monitor integrations and failures [file:1] | Needed for production debugging and operational trust [file:1] |
| Security controls | Platform-wide protection rules | JWT auth, RBAC, RLS, HMAC webhook validation, audit rules, data retention [file:1] | Mandatory [file:1] | Defense in depth, not single-layer security [file:1] | Protects customer data and platform reliability [file:1] |
| External integrations | Systems connected to the platform | Salesforce, HubSpot, Dynamics 365, Zoom, Meet, Teams, Gmail, Outlook, Slack, LinkedIn, Snowflake, BigQuery and others [file:1] | Mix of deployed and planned by phase [file:1] | Use approved auth and idempotency patterns [file:1] | These integrations create the platform’s real business value [file:1] |
| External AI dependencies | Third-party AI services the product relies on | OpenAI API, Whisper, AssemblyAI [file:1] | Approved and active [file:1] | Must be called only from Python services [file:1] | These power core AI features and cost center tracking [file:1] |
| Testing strategy | Full quality strategy across layers | Jest, pytest, Supertest, Playwright, Testcontainers, k6 + AI evaluation methods [file:1] | Approved / recommended [file:1] | Unit tests are not enough; event-flow testing required [file:1] | Ensures both correctness and production readiness [file:1] |
| AI quality control | How AI outputs are validated | Structured JSON, confidence score, review gating, golden datasets, prompt versioning [file:1] | Required for safe AI rollout [file:1] | Low-confidence outputs should be reviewed [file:1] | Improves trust and reduces bad automation [file:1] |
| Tool governance | How tools are approved or rejected | Approved / In Use / Planned / Optional / Rejected model [file:1] | Governance process defined [file:1] | Tech Lead approval required for shared tools [file:1] | Prevents tool sprawl and inconsistent engineering practices [file:1] |
| Payment planning | How upgrade decisions are made | Usage-based and trigger-based upgrades across infra, AI, observability, and search tools [file:1] | Planned and controlled [file:1] | Upgrade only when speed, reliability, scale, or security justifies it [file:1] | Helps control burn while staying production-ready [file:1] |
| Risks | Main architectural and operational risks | Vendor lock-in, API costs, free-tier limits, privacy risks, migration risks, security risks [file:1] | Active and documented [file:1] | Risks must be monitored per phase [file:1] | Prevents naive architecture and budget decisions [file:1] |
| Data ownership | Who owns customer data | Client owns data and can export it; no training without consent [file:1] | Mandatory rule [file:1] | Data Cloud exports to client-owned destinations [file:1] | Critical for trust, compliance, and enterprise readiness [file:1] |
| Change governance | How the document and platform evolve | PR-based updates, Tech Lead approval, 3-month review cadence [file:1] | Mandatory [file:1] | Update SAD for any shared architecture change [file:1] | Keeps the architecture as the single source of truth [file:1] |

### Final summary note

In simple terms, this document says the team should build **one clean modular platform now**, keep **all boundaries disciplined from day one**, use **TypeScript for product logic**, use **Python for AI**, protect **tenant data very aggressively**, avoid **premature microservices**, and only unlock downstream modules when upstream events are working with real production data. That is the safest way to let freshers build fast without breaking the long-term architecture. [file:1]