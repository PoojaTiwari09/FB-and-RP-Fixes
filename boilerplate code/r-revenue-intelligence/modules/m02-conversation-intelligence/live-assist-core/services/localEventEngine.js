...
export function shouldTriggerLLM(signalDelta, cooldowns, lastLLMCall, currentTranscript, lastAnalyzedTranscript, turnTransition = false) {
  ...
  if (signalDelta.hasNewCompetitor && timeSinceLastLLM > 10000) {
    return { trigger: true, reason: 'competitor_detected', requestType: 'full', isTurn: false };
  }

  if (signalDelta.hasCriticalEvent && timeSinceLastLLM > 12000) {
    return { trigger: true, reason: 'critical_event', requestType: 'full', isTurn: false };
  }

  // ── TRIGGER 2: TURN TRANSITION (MOST IMPORTANT) ──
  // If the client finished speaking and it's been at least 3 seconds since our last tactical generation
  // Reduced cooldown from 8 seconds to 3 seconds for instant suggested responses
  if (turnTransition && timeSinceLastLLM > 3000) {
  ...
  if (signalDelta.deltaScore >= 8 && timeSinceLastLLM > 10000) {
    return { trigger: true, reason: 'signal_delta_spike', requestType: 'full', isTurn: false };
  }

  // ── BASELINE REFRESH (prevent stale suggestions) ──
  if (timeSinceLastLLM > 20000 && currentTranscript !== lastAnalyzedTranscript) {
    return { trigger: true, reason: 'baseline_refresh', requestType: 'standard', isTurn: false };
  }
