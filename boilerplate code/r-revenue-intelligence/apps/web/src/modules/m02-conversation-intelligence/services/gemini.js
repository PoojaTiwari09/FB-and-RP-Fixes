...
export async function generateLiveAssistInsights(keys, payload) {
  ...
  // Run both paths in parallel for speed, UNLESS it's a tactical_only turn-transition request
  const runReasoning = payload.requestType !== "tactical_only";

  const [fastResult, reasoningResult] = await Promise.allSettled([
    // Fast path: Groq Llama 3.1 8B (ALWAYS RUNS)
    groqKey
      ...
      : Promise.reject(new Error("No Groq key for fast path")),

    // Reasoning path: GPT-4o-mini via OpenRouter (SKIPPED on tactical_only)
  ...
  if (!fastParsed.suggestedResponses && !reasoningParsed.headline) {
