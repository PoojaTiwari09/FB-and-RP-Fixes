...
export default function useSupabaseData() {
  ...
  async function addCall(callData, apiKey) {
    ...
    if (sb) {
      // 1. Overwrite / Deduplication: Delete existing call transcript if it exists
      // foreign key 'on delete cascade' will automatically clean up transcript_chunks!
      await sb.from("calls").delete().eq("id", callId);

      // 2. Insert Call Transcript
      const { data, error: callError } = await sb.from("calls").insert(newCall).select().single();
      if (callError) throw callError;

      // 3. Generate and Insert Transcript Chunks & Embeddings (RAG Ingestion)
      ...
      // Remove old call in state to avoid duplicates
