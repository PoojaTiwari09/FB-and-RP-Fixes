...
import type { AIGeneratedSummary, BriefSection, RichCitation } from "@/types/database";
import clsx from "clsx";
import {
  ChevronDown, ChevronUp, BookOpen, ShieldCheck,
  Clock, AlertCircle, Loader2, FileText,
} from "lucide-react";
import { EvidenceButton, CitationPopup } from "@/components/workspace/CitationPopup";
...
const isPlaceholder = (t: string) => PLACEHOLDER.some(p => p.test(t));
...
function SentimentBadge({ value }: { value?: string }) {
  ...
  const isPos = v.includes("positive") || v.includes("good");
  const isNeg = v.includes("negative") || v.includes("poor") || v.includes("risk");
  return (
    <span className={clsx(
...
function BulletRow({ text, richCitations }: { text: string; richCitations?: RichCitation[] }) {
...
export function SharedBriefView({ token }: SharedBriefViewProps) {
  const [summary,   setSummary]   = useState<AIGeneratedSummary | null>(null);
  const [meta,      setMeta]      = useState<{ briefType: string; expiresAt: string | null; createdAt: string } | null>(null);
  const [loading,   setLoading]   = useState(true);
  const [error,     setError]     = useState<string | null>(null);
  const [activeCit, setActiveCit] = useState<{ citation: RichCitation; rect: DOMRect } | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setSummary(body.data.summary as AIGeneratedSummary);
        setMeta({
