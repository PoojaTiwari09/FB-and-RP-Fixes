...
export function buildSystemPrompt(contextChunks) {
  const contextStr = contextChunks.map(c => {
    ...
    if (c.type === "transcript_chunk") {
      const tc = c.data;
      if (tc.source_type === "email" || (tc.id && String(tc.id).startsWith("EMAIL_"))) {
        return `EMAIL RECORD [Subject: ${tc.call_title || "Email"}]: "${tc.chunk_text}"`;
      }
...
- Behave like a sales coach and account strategist. Ensure the tone is consultative and actionable.
...
If the user asks questions comparing/analyzing multiple recent discussions:
...
If the user specifies a timeframe:
1. Restrict retrieval and reasoning to ONLY that timeframe.
2. Explicitly mention the timeframe being analyzed in the response.

---

## ACCOUNT HANDOVER BEHAVIOR
If the user asks questions regarding onboarding/history:
...
1. Never hallucinate facts not present in the retrieved data.
