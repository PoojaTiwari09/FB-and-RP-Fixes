...
import useSupabaseData from "../modules/hooks/useSupabaseData";
...
export default function App() {
  ...
  const [chatSessionId, setChatSessionId]   = useState(""); // Initialize on mount to prevent SSR timestamp mismatch
  ...
  function handleDealSelect(deal) {
    if (activeDeal?.id === deal.id) {
      ...
      setActiveDeal(deal);
      // Auto-select associated account if any
  ...
  function handleAccountSelect(acc) {
    if (activeAccount?.id === acc.id) {
      ...
      setActiveDeal(null); // Clear deal focus, general account search
      setActiveChat(null);
      setActiveChatId(null);
    }
  }
  ...
  function handleChatSelect(chat) {
    ...
    // Clear deal/account filter context when reviewing history
    setActiveDeal(null);
    setActiveAccount(null);
