...
import type { Role, FilterCondition } from '@/modules/m05-account-intelligence/types';
...
export const useSessionStore = create<SessionState>()(
  persist(
    ...
    }),
    {
      name: 'rid-session',
      version: 2,
      // Strip stale activeBoard/activeTabId from old localStorage data
      migrate: (persistedState: any, version: number) => {
        if (version < 2 && persistedState) {
          delete persistedState.activeBoard;
      ...
      partialize: (state) => ({
        role: state.role,
        // NOTE: activeBoard and activeTabId are intentionally NOT persisted.
        // activeBoard comes from the URL slug, activeTabId is validated by the
        // board page against the actual board config. Persisting them caused
        // stale state to redirect users to the wrong board.
        period: state.period,
