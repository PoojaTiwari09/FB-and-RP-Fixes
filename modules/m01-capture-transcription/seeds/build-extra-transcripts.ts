/**
 * Transcribe extra S3 demo recordings via AssemblyAI and merge into demo-transcript-bundle.json.
 * Run from backend root:
 *   pnpm exec tsx modules/m01-capture-transcription/seeds/build-extra-transcripts.ts
 */
...
async function submitTranscript(audioUrl: string, speakersExpected = 2): Promise<string> {
...
function mapSpeaker(raw: string, speakerMap: Map<string, 'Rep' | 'Customer'>): 'Rep' | 'Customer' {
  if (speakerMap.has(raw)) return speakerMap.get(raw)!;
  const role: 'Rep' | 'Customer' = speakerMap.size === 0 ? 'Rep' : 'Customer';
  speakerMap.set(raw, role);
  return role;
}

function splitMonologueIntoDialog(fullText: string, durationMs: number) {
  const sentences =
    fullText.match(/[^.!?]+[.!?]+(?:\s|$)/g)?.map((s) => s.trim()).filter(Boolean) ?? [fullText.trim()];
  if (sentences.length <= 1) return null;

  const msPerSentence = durationMs / sentences.length;
  const utterances = sentences.map((text, sequenceIndex) => ({
    speaker: (sequenceIndex % 2 === 0 ? 'Rep' : 'Customer') as 'Rep' | 'Customer',
    text,
...
function buildBundle(result: any) {
  const speakerMap = new Map<string, 'Rep' | 'Customer'>();
  let utterancesRaw: AaiUtterance[] = result.utterances ?? [];

  if (utterancesRaw.length <= 2 && Array.isArray(result.words) && result.words.length > 0) {
    const grouped: AaiUtterance[] = [];
    let current: AaiUtterance | null = null;
    ...
    if (current) grouped.push(current);
    if (grouped.length > 2) utterancesRaw = grouped;
  }
  ...
  const durationMs = Math.max(
    result.audio_duration ? result.audio_duration * 1000 : 0,
  ...
  if (utterances.length <= 2 && (result.text?.length ?? 0) > 400) {
    ...
    if (split && split.length > 2) utterances = split;
  }
...
async function main() {
  const bundlePath = join(__dirname, 'demo-transcript-bundle.json');
