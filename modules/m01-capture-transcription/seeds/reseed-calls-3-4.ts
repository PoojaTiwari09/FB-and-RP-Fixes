/**
 * Re-seed transcripts for demo calls 3 & 4 only (after AssemblyAI bundle update).
 * Run: pnpm exec tsx modules/m01-capture-transcription/seeds/reseed-calls-3-4.ts
 */
...
const [CALL_3, CALL_4] = DEMO_CALL_IDS.slice(2);
...
async function reseedCall(c: (typeof CALLS)[0]) {
  ...
  console.log(
    `  ${c.id} — ${c.bundle.utterances.length} utterances, ${c.bundle.durationSeconds}s`,
  );
}
