...
exports.SchedulerService = exports.AnalyticsService = exports.TrainingService = exports.CoachingService = exports.ScenariosService = exports.SessionsService = exports.LlmService = void 0;
...
let LlmService = class LlmService {
    constructor(configService) {
        ...
        const read = (k) => this.configService?.get?.(k) || process.env[k] || '';
        ...
        this.groq = new groq_sdk_1.default({ apiKey: this.groqApiKey });
        const resolved = (0, llm_provider_factory_1.resolveLlmProviderKind)();
    ...
    buildMockBuyerResponse(userMessage, history, systemPrompt) {
        ...
        const prevReplies = history.filter((m) => m.role === 'assistant').map((m) => (m.content || '').toLowerCase());
        const alreadyMentioned = (keyword) => prevReplies.some(r => r.includes(keyword));
        let hashVal = 0;
        for (let i = 0; i < msgRaw.length; i++)
            hashVal = ((hashVal << 5) - hashVal + msgRaw.charCodeAt(i)) | 0;
        ...
        if (isQuestion) {
            const openTopics = [];
            if (!alreadyMentioned('pric') && !alreadyMentioned('cost'))
                openTopics.push('total cost');
            if (!alreadyMentioned('timeline'))
                openTopics.push('realistic timeline');
            if (!alreadyMentioned('roi'))
                openTopics.push('measurable ROI');
            if (!alreadyMentioned('team') && !alreadyMentioned('adoption'))
                openTopics.push('team adoption');
    ...
    buildMockLiveEvaluation(lastUserMsg, history = []) {
        ...
        const repMessages = history.filter((m) => m.role === 'user');
        const assistantMessages = history.filter((m) => m.role === 'assistant');
        const repWords = repMessages.map((m) => m.content || '').join(' ').split(/\s+/).filter(Boolean).length + lastUserMsg.split(/\s+/).filter(Boolean).length;
        const assistantWords = assistantMessages.map((m) => m.content || '').join(' ').split(/\s+/).filter(Boolean).length;
    ...
    async generateHint(history, scenarioContext, hintsUsed = 0) {
        if (this.aiMockMode) {
            const mockHints = [
        ...
        try {
            ...
            return `Groq API Error: ${error.message || 'Failed to generate hint'}`;
        }
    }
    async generateBuyerResponse(systemPrompt, history, userMessage) {
        if (this.aiMockMode) {
    ...
    async evaluateSession(evaluationPrompt) {
        if (this.aiMockMode) {
    ...
    async generateCoachingActions(analysisPrompt) {
        if (this.aiMockMode) {
    ...
    async generatePersonaFromTranscript(transcript) {
        if (this.aiMockMode) {
            return this.enrichPersonaDraft({
                persona_name: 'Budget-Constrained Bob',
                persona_type: 'Budget Constrained',
                difficulty: 'intermediate',
                context_text: 'Bob is a car owner contacted about a map update. He is hesitant because of the price and wants to understand the value before paying.',
                objectives: '',
                goals: '',
                custom_prompt: '',
                evaluation_focus: '',
        ...
        try {
            const completion = await this.groq.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are an expert sales trainer. Analyze sales call transcripts and design roleplay training scenarios. Return ONLY valid JSON, no markdown.',
                    },
                    {
                        role: 'user',
  ...
  "personality_traits": "3-5 traits describing how they communicate"
    ...
    enrichPersonaDraft(persona, transcript = '') {
        ...
        const defaults = typeDefaults[type] || {
            objectives: '• Run discovery on needs, timeline, and decision process\n• Present tailored value\n• Handle objections\n• Confirm next steps',
        ...
        const context = fill(persona.context_text, transcript
        ...
        return {
            ...
            custom_prompt: fill(persona.custom_prompt, `You are ${name}, a ${type} buyer in roleplay. Object like: ${defaults.objection_style}. Style: ${defaults.personality_traits}. Stay in character. 1-3 sentences per turn. Context: ${context.slice(0, 300)}`),
            evaluation_focus: fill(persona.evaluation_focus, defaults.evaluation_focus),
            target_skills: Array.isArray(persona.target_skills) && persona.target_skills.length
    ...
    async evaluateLiveTurn(history, lastUserMsg, scenarioContext = '') {
        if (this.aiMockMode) {
        ...
        try {
            ...
            const repMessages = history.filter((m) => m.role === 'user');
            const repWords = repMessages.map((m) => m.content || '').join(' ').split(/\s+/).filter(Boolean).length;
            const totalWords = history.map((m) => m.content || '').join(' ').split(/\s+/).filter(Boolean).length;
...
Evaluate this latest turn only and return the strict JSON payload.`
    ...
    async generateLiveCoachingTips(history, lastUserMsg, lastBotReply) {
        if (this.aiMockMode) {
    ...
    async diarizeTranscript(rawTranscript) {
        if (this.aiMockMode) {
...
let SessionsService = class SessionsService {
    constructor(repository, llmService, worker) {
        this.repository = repository;
        this.llmService = llmService;
        this.worker = worker;
    }
    async findAll(repId, orgId) {
        return this.repository.findAllSessions(repId, orgId);
    }
    async getMySessions(repId, orgId) {
    ...
    async getSessionById(sessionId, orgId) {
    ...
    async startSession(dto, repId, orgId) {
    ...
    async getHint(sessionId, orgId) {
        ...
        if (!session.is_practice) {
            ...
            if (session.hints_used >= maxHints) {
                throw new common_1.BadRequestException('Hint limit reached for this assignment.');
            }
        }
...
${assistantHistory.length > 0 ? assistantHistory.map((m, i) => `  Turn ${i + 1}: "${m.content}"`).join('\n') : '  (nothing yet — this is your opening response)'}
...
5. Respond ONLY to what the rep just said — don't ignore it and jump to a pre-planned script`;
    ...
    async sendVoiceMessage(sessionId, audio, orgId) {
        if (!audio)
            throw new common_1.BadRequestException('No audio file');
        ...
        try {
            ...
            if (!userText)
                throw new common_1.BadRequestException('Transcription failed');
    ...
    async endSession(sessionId, orgId) {
        ...
        const repWords = repMessages.map((m) => m.content || '').join(' ').split(/\s+/).filter(Boolean).length;
        const botWords = botMessages.map((m) => m.content || '').join(' ').split(/\s+/).filter(Boolean).length;
        const totalWords = repWords + botWords;
        const actualTalkRatio = totalWords > 0 ? Math.round((repWords / totalWords) * 100) : 0;
        const questionsAsked = repMessages.filter((m) => (m.content || '').includes('?')).length;
        const closingKeywords = ['next step', 'move forward', 'schedule', 'demo', 'trial', 'sign', 'ready to'];
        const closingAttempts = repMessages.filter((m) => closingKeywords.some(k => (m.content || '').toLowerCase().includes(k))).length;
...
Return ONLY JSON:
{
  ...
  "improvements": ["improvement"]
    ...
    async retrySession(sessionId, repId, orgId) {
        const session = await this.repository.findSessionById(sessionId, orgId);
        const assignment = await this.repository.findAssignmentBySessionId(sessionId);
        if (!assignment)
            throw new common_1.BadRequestException('No active assignment found for this session');
    ...
    async updateSession(id, dto, orgId) {
        if (dto.messages_json) {
            await this.repository.updateSessionMessages(id, dto.messages_json);
        }
        return { success: true };
    }
    async submitSessionToManager(sessionId, repId, orgId) {
        const session = await this.repository.findSessionById(sessionId, orgId);
        if (session.rep_id !== repId) {
            throw new common_1.BadRequestException('You can only submit your own sessions.');
        }
        const assignment = await this.repository.findAssignmentForRepSession(sessionId, repId, session.scenario_id);
        if (!assignment) {
            throw new common_1.BadRequestException('No training assignment found for this session. Start from Assignments to submit to your manager.');
        }
    ...
    async analyzeUploadedCall(repId, orgId, audio) {
        if (!audio)
        ...
        try {
            ...
            const transcript = !rawTranscript || rawTranscript.trim().length < 20
            ...
            if (!scenario) {
                throw new common_1.BadRequestException('No training scenario found in your org. Please create at least one scenario first.');
            }
            const session = await this.repository.createSession({
                rep_id: repId,
...
FULL TRANSCRIPT:
${messages.map((m) => `${m.role === 'user' ? 'REP' : 'PROSPECT'}: ${m.content}`).join('\n')}
...
exports.SessionsService = SessionsService = __decorate([
    (0, common_1.Injectable)(),
    __param(2, (0, common_1.Inject)((0, common_1.forwardRef)(() => m09_worker_1.M09Worker))),
...
let ScenariosService = class ScenariosService {
    ...
    async create(dto, orgId, managerId) {
        const metadata = {
    ...
    async analyzeAudioForScenario(audioBuffer) {
        const ext = '.webm';
        ...
        try {
            ...
            const transcript = messages
                ...
                .join('\n\n');
            const persona = await this.llmService.generatePersonaFromTranscript(transcript || rawTranscript);
            return {
...
let CoachingService = class CoachingService {
    ...
    async getNotes(userId, role, orgId) {
    ...
    async createNote(managerId, repId, content, priority = 'Medium', orgId) {
        const scenarios = await this.repository.findAllScenarios(orgId);
        const scenarioId = scenarios?.[0]?.id;
        if (!scenarioId)
            throw new common_1.BadRequestException('Create a scenario in this organization first.');
    ...
    async getRecommendations(repId) {
        return this.repository.findRecommendations(repId);
    }
    async pushRecommendation(managerId, repId, focusArea, text) {
...
let TrainingService = class TrainingService {
    ...
    async createAssignments(dto, managerId, orgId) {
        const scenario = await this.repository.findScenarioById(dto.scenarioId, orgId);
        if (!scenario)
            throw new common_1.NotFoundException('Selected scenario not found');
    ...
    async updateAssignment(id, dto, managerId) {
    ...
    async updateAssignmentByRep(id, dto, repId) {
        const assignment = await this.repository.findAssignmentById(id);
        if (!assignment || assignment.rep_id !== repId) {
            throw new common_1.BadRequestException('Assignment not found or access denied.');
        }
        const updates = {};
        if (dto.status)
...
let AnalyticsService = class AnalyticsService {
    ...
    async getDashboardStats(orgId, managerId) {
        ...
        sessions.forEach(s => {
            const f = s.feedback_json;
            if (f?.is_note || typeof f?.overall_score !== 'number')
                return;
        ...
        sessions.forEach(s => {
            ...
            if (d >= oneWeekAgo) {
                thisWeekScore += f.overall_score;
                thisWeekCount++;
            }
            else if (d >= twoWeeksAgo && d < oneWeekAgo) {
                lastWeekScore += f.overall_score;
                lastWeekCount++;
            }
        });
    ...
    async getRepsWithStats(orgId, managerId) {
        const reps = await this.repository.getRepsByOrgId(orgId, managerId);
        const repIds = reps.map(r => r.id);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
        return reps.map(rep => {
            const repSessions = sessions.filter(s => s.rep_id === rep.id && !s.feedback_json?.is_note);
            const scores = repSessions.map(s => s.feedback_json?.overall_score || 0);
    ...
    async getTeamAnalytics(orgId, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
        ...
        const scenarioMap = {};
        sessions.forEach(s => {
            if (s.feedback_json?.is_note)
                return;
            const name = s.scenario?.persona_name || 'Unknown';
            if (!scenarioMap[name])
                scenarioMap[name] = [];
            scenarioMap[name].push(s.feedback_json?.overall_score || 0);
        });
    ...
    async getRepComparison(repId, orgId, managerId) {
        const teamRepIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        if (!teamRepIds.includes(repId)) {
            throw new common_1.BadRequestException('Rep not found in your team');
        }
    ...
    async getActivityMetrics(orgId, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
        const totalSessions = sessions.filter(s => !s.feedback_json?.is_note).length;
        return {
            totalSessions,
    ...
    async getInteractionAnalytics(orgId, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
    ...
    async getTopicInsights(orgId, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
    ...
    async getCallDrilldown(sessionId, orgId) {
        const session = await this.repository.findSessionById(sessionId, orgId);
        const assignment = await this.repository.findAssignmentBySessionId(sessionId);
        return {
            id: session.id,
    ...
    async getBenchmarks(orgId, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
    ...
    async getManagerReview(managerId, orgId) {
    ...
    async getTrainingReport(orgId, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        const sessions = await this.repository.getSessionsByRepIds(repIds);
        const assignments = await this.repository.getAssignmentsByRepIds(repIds);
        const completed = assignments.filter(a => a.completed_at).length;
    ...
    async exportCsv(orgId, options, managerId) {
    ...
    async exportTrainingCsv(orgId, options, managerId) {
        const repIds = await this.repository.getRepIdsByOrgId(orgId, managerId);
        ...
        if (options.dateRange) {
            ...
            if (options.dateRange === 'last_7_days')
            ...
            else if (options.dateRange === 'last_90_days')
                threshold = new Date(now.setDate(now.getDate() - 90));
    ...
    async getSentNotes(managerId, orgId) {
        return this.getNotes(managerId, 'manager', orgId);
    }
    async getTeamAssignments(managerId, orgId) {
        return this.repository.findAssignmentsByManager(managerId, orgId);
    }
    async getMyAnalytics(userId, orgId) {
        const sessions = await this.repository.getSessionsByRepIds([userId]);
        return sessions.filter((s) => s.feedback_json && !s.feedback_json.is_note);
    }
    async getMyNotes(userId, orgId) {
        return this.getNotes(userId, 'rep', orgId);
    }
    async getMyAssignments(userId, orgId) {
...
let SchedulerService = class SchedulerService {
    constructor(repository) {
        ...
        this.REFRESH_INTERVAL_MS = 12 * 60 * 60 * 1000;
    }
    onModuleInit() {
...
//# sourceMappingURL=m09.service.js.map