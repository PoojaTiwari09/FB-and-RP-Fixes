...
const service = new ForecastUpgradeService(prisma as any);
...
async function runE2E() {
  ...
  const PERIOD_ID = '00000000-0000-0000-0000-0000000000b2'; // Q2 FY26

  try {
    // 1. Fetch periods
    console.log('[1/7] Fetching Forecast Periods...');
    const periods = await service.getPeriods();
    if (!periods.find((p) => p.id === PERIOD_ID)) throw new Error('Period not found');
    console.log('✅ Periods loaded successfully');

    // 2. Fetch Rep Drilldown and Summary
    ...
    if (targetCheck?.amount !== 8000000) throw new Error('Target assignment failed');
