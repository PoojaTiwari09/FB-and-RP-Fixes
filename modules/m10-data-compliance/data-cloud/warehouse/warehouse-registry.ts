...
import type {
  WarehouseProvider,
  WarehouseSyncContext,
  WarehouseSyncResult,
} from "./warehouse-provider.interface";
...
class SnowflakeWarehouseProvider implements WarehouseProvider {
  ...
  async sync(ctx: WarehouseSyncContext): Promise<WarehouseSyncResult> {
    ...
    return {
      ...
      destination: "snowflake",
      detail:
        "Snowflake COPY hook acknowledged (connector stub — files ready for ingest)",
...
class BigQueryWarehouseProvider implements WarehouseProvider {
  ...
  async sync(ctx: WarehouseSyncContext): Promise<WarehouseSyncResult> {
    ...
    return {
      ...
      destination: "bigquery",
      detail:
        "BigQuery load job hook acknowledged (connector stub — files ready for ingest)",
...
export class WarehouseRegistry {
  ...
  resolve(destination: string): WarehouseProvider {
    return (
      this.providers.find((p) => p.supports(destination)) ?? this.providers[0]
    );
  }
