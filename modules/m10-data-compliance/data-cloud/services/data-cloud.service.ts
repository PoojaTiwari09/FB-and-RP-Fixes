...
import type {
  ...
  ReplayExportDto,
} from "../schemas/data-cloud.schema";
...
export class DataCloudService {
  ...
  constructor(
    ...
    this.redis = new Redis(process.env.REDIS_URL ?? "redis://localhost:6379", {
      ...
      retryStrategy: (times) => {
        if (process.env.DISABLE_REDIS === "true") return null; // stop retrying
        return Math.min(times * 300, 10000);
      },
    });
    this.redis.on("error", () => {}); // swallow connection errors — Redis is optional
  }
  ...
  async getExportRuns(tenantId: string, connectionId?: string): Promise<any[]> {
    ...
    return runs.map((r) => ({
      ...
      downloadUrls: this.buildDownloadUrls(
        tenantId,
        r.id,
        r.filePaths as Record<string, any> | null,
      ),
    }));
  }
  ...
  async getExportDownloadStream(
    ...
    const path = this.storage.resolveDownloadPath(
      tenantId,
      runId,
      dataset,
      format,
  ...
  async runScheduledExport(
    ...
    if (existing) {
      this.logger.warn(
        `Export aborted: lock held tenant=${tenantId} conn=${connectionId}`,
      );
      return;
    }
    ...
    if (!run)
      throw new HttpException("Export run not found", HttpStatus.NOT_FOUND);
  ...
  private buildDownloadUrls(
    tenantId: string,
    runId: string,
    filePaths: Record<string, any> | null,
  ) {
    if (!filePaths) return {};
    ...
    for (const ds of Object.keys(filePaths)) {
      out[ds] = {
        csv:
          filePaths[ds]?.downloadCsv ??
          this.storage.downloadUrl(runId, ds, "csv"),
        parquet:
          filePaths[ds]?.downloadParquet ??
