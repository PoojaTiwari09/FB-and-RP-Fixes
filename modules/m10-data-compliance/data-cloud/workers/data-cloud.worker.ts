...
export class DataCloudWorker extends WorkerHost {
  ...
  async process(job: Job): Promise<void> {
    this.logger.log(
      `Processing export job [id=${job.id}] [attempt=${job.attemptsMade + 1}]`,
    );
    ...
    await this.service.runScheduledExport(
      tenantId,
      connectionId,
      (job.data as any).runId,
    );
    this.logger.log(
      `Export job [id=${job.id}] completed for tenant=${tenantId}`,
    );
  }
