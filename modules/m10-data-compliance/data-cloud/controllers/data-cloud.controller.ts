...
export class DataCloudController {
  ...
  async registerConnection(
    ...
    this.logger.log(
      `POST exports/connections — tenant=${req.tenantId} dest=${parsed.data.destination}`,
    );
  ...
  async testConnection(
    ...
    this.logger.log(
      `POST exports/connections/${id}/test — tenant=${req.tenantId}`,
    );
  ...
  async triggerReplay(@Req() req: AuthenticatedRequest, @Body() body: unknown) {
    ...
    this.logger.log(
      `POST exports/replay — tenant=${req.tenantId} dataset=${parsed.data.datasetName}`,
    );
  ...
  async downloadExport(
    ...
    const { stream, path, contentType } =
      await this.service.getExportDownloadStream(
        req.tenantId,
        runId,
        dataset,
        format,
