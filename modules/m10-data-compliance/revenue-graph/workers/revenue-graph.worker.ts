...
import {
  M10_REVENUE_GRAPH_QUEUES,
  M10_REVENUE_GRAPH_EVENTS,
} from "../events/revenue-graph.events";
import {
...
export class RevenueGraphWorker extends WorkerHost {
  ...
  async process(job: Job): Promise<void> {
    this.logger.log(
      `Processing job [id=${job.id}] [name=${job.name}] [attempt=${job.attemptsMade + 1}]`,
    );

    // Only handle events this worker owns
    if (
      job.name !==
      M10_REVENUE_GRAPH_EVENTS.CONSUMED.CALL_TRANSCRIPTION_COMPLETED
    ) {
    ...
    if (!parsed.success) {
      this.logger.error(
        `Job [id=${job.id}] invalid payload: ${JSON.stringify(parsed.error.flatten())}`,
      );
      throw new Error(
        `UNRECOVERABLE: Invalid event payload — ${JSON.stringify(parsed.error.flatten())}`,
      );
    }
    ...
    if (!event.tenantId) {
      ...
      throw new Error(
        "UNRECOVERABLE: tenantId is required for Revenue Graph linking",
      );
    }
  ...
  onFailed(job: Job | undefined, error: Error): void {
    ...
    if (isUnrecoverable) {
      this.logger.error(
        `Job [id=${job?.id}] is unrecoverable — will NOT be retried`,
      );
    }
  }
