...
export default function TakeActionDrawer({ task, onClose, onEmail, onMessage }: TakeActionDrawerProps) {
  ...
  return (
    <>
      ...
      <div
        onClick={onClose}
        className="fixed inset-0 z-40 bg-black/20 transition-opacity animate-fade-in"
      />

      {/* Sidebar Panel */}
      <div
        className="fixed top-0 right-0 z-50 h-full w-[400px] bg-white border-l border-gray-200 shadow-2xl flex flex-col overflow-hidden transform transition-transform animate-slide-in"
