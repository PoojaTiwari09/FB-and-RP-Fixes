...
*   **Special Domain Rules & Constraints:**
    ...
    *   **Domain Field Mappings:** The database mapping layer resolves discrepancies between domain definitions and database schema columns (e.g. domain field `isHighRisk` is mapped to database column `escalated`, and domain field `riskReason` is mapped to database column `riskLabel` on `Deal` records).
