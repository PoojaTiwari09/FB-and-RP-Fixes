...
**Architecture (ports, DB per module):** [ARCHITECTURE.md](./ARCHITECTURE.md)

**API documentation:** `../../doc/execution/api-docs/README.md`

Run each module **individually** (separate backend + frontend).

**Repo root** (clone root — `docker compose`):

```powershell
cd <path-to-r-revenue-intelligence-monorepo>
docker compose up -d
```

**App folder** (all `pnpm` commands):
...
**`EPERM` on `pnpm run db:generate`:** a dev server (e.g. `dev:m01-api`, `dev:m02-web`) has locked Prisma’s Windows DLL. Stop all module servers, then:
...
You only need `db:generate` after schema changes or a fresh clone — not on every `dev:mXX` start.
